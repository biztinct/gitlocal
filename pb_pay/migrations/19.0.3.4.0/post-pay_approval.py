# -*- coding: utf-8 -*-
"""Approval Matrix P6 — pb_pay's default approval route, on an upgrade.
...
def migrate(cr, version):
    ...
    from odoo.addons.pb_pay.models.pb_pay_approval import seed_all
    try:
        seed_all(env)
    except Exception:       # noqa: BLE001 — an upgrade must not die on a seed
        _logger.exception('pb_pay: the default approval route could not '
                          'be seeded')
