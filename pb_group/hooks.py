...
`post_init_hook` runs on INSTALL only, so the migration beside it does the
same job on `-u` (ledger AM70/AM91). Both are idempotent.
"""
...
def post_init_hook(env_or_cr, registry=None):
    ...
    try:
        from .models.fx_approval import seed_all
        seed_all(env)
