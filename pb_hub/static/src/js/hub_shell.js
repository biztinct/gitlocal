...
export class HubShell extends Component {
    ...
    static components = { HubBackChip, HubTracker };
    static props = {
        /**
    ...
    async _resolveAccess() {
        ...
        // stale deep link would land them on an error state.
        if (!allowed[this.state.lens]) {
            const first = lenses.find((l) => allowed[l.key]);
            this.state.lens = first ? first.key : "";
        }
    }
    ...
    setLens(key) {
        const keys = (this.config.lenses || []).map((l) => l.key);
        if (!keys.includes(key) || this.state.lens === key) { return; }
