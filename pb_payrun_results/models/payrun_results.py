...
class PayrunResults(models.AbstractModel):
    ...
    @staticmethod
    def _num(x):
        """Coerce to float; None for non-numeric (some components emit text —
        names, labels — which must not break totals or the number formatting)."""
        if x is None or x == '':
            return None
        try:
            return float(x)
    ...
    def _columns(self, config):
        """Report columns = the config's on-payslip / report-visible components,
        in display (sequence) order — after F111 that is the user's curated
        order, so the grid inherits grouping for free."""
        cols, cat_color, seen = [], {}, 0
        ...
        for r in rules:
            ...
            cols.append({
                ...
                'is_deduction': (r.category_id.name or '').lower().find('deduc') >= 0
                                if r.category_id else False,
            })
        return cols
    ...
    def _formula_runs(self):
        ...
        first. One search; batch the has-formula check."""
        Run = self.env['hr.payslip.run']
        runs = Run.search([], order='id desc', limit=200)
        out = []
        for run in runs:
            if any(s.formula_computed_values for s in run.slip_ids[:5]) or \
               self.env['hr.payslip'].search_count(
                   [('payslip_run_id', '=', run.id), ('formula_computed_values', '!=', False)]):
                out.append({'id': run.id, 'name': run.name or _('Run #%s') % run.id})
            if len(out) >= 60:
                break
        return out
    ...
    # ------------------------------------------------------------------
    def _pair_variance(self, page_slips, codes, parsed, config):
        if not page_slips:
            return {}
        cycle = config.cycle_type if config else False
        ...
        if cycle:
            dom.append(('formula_config_id.cycle_type', '=', cycle))
        prev = self.env['hr.payslip'].search(dom, order='employee_id, date_to desc')
        latest = {}
        for p in prev:
            latest.setdefault(p.employee_id.id, p)
        out = {}
        for s in page_slips:
            p = latest.get(s.employee_id.id)
            if not p:
                continue
            try:
    ...
    def get_grid(self, run_id=None, filters=None):
        self = self.sudo()   # read-only reporting over payroll data (cockpit pattern)
        f = filters or {}
        ...
        config = slips[:1].formula_config_id
        if not slips or not config:
        ...
        deltas = self._pair_variance(page_slips, codes, parsed, config) if f.get('with_variance') else {}
        prev_run_label = None
        if f.get('with_variance') and deltas:
            prev_run_label = _('same-cycle previous slip per employee')
        ...
        return {
            'ok': True,
            'run': {'id': run.id, 'name': run.name or '', 'state': run.state,
                    'config_name': config.name or '', 'currency': (config.currency_id.symbol or '₫')},
            'runs': runs,
            'columns': cols,
            'rows': rows,
            'totals': totals,
