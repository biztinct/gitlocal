...
/** Biztinct */
topbarMenuRegistry.add("payroll", {name: _t("Payroll"), sequence: 9});
/**End */
...
topbarMenuRegistry.addChild("copy_sheet_as_values", ["file"], {
    ...
    action: (env) => env.copySheetAsValues(), // Use the registered method in env
});
*/

topbarMenuRegistry.addChild("copy_sheet_as_values", ["payroll"], {
    name: "Step 1 - Employee Details",
    sequence: 30, // Adjust position in the menu
    action: (env) => env.copySheetAsValues(), // Call the registered action
});

topbarMenuRegistry.addChild("copy_mastersheet_as_values", ["payroll"], {
    name: "Step 2 - Pay Calculations",
    sequence: 40, // Adjust position in the menu
    action: (env) => env.copyMasterSheetAsValues(), // Call the registered action
});

topbarMenuRegistry.addChild("copy_finalsheet_as_values", ["payroll"], {
    name: "Step 3 - Prepare to import",
    sequence: 50, // Adjust position in the menu
    action: (env) => env.createFinalSheetAsValues(), // Call the registered action
});
...
export class EditFilterPanel extends Component {
    setup() {
        ...
        this.state = useState({
            ...
            defaultsToCurrentPeriod: this.props.filter.defaultsToCurrentPeriod || true,
    ...
    /** Biztinct - no need now
    onDateRangeChange(ev) {
        const selectedValue = ev.target.value;
        if (!selectedValue) {
            // Get the current month and year
            const today = new Date();
            const currentMonth = String(today.getMonth() + 1).padStart(2, '0'); // Adding 1 as getMonth() is zero-indexed
            const currentYear = today.getFullYear();
    
            // Pre-fill the filter with the current month and year
            this.state.rangeType = `${currentMonth}/${currentYear}`;
        } else {
            // Update state with selected value
            this.state.rangeType = selectedValue;
        }
        // Trigger re-render or additional actions
        this.render();
    }
    */
