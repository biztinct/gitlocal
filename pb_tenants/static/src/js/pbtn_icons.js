/** @odoo-module **/
// Module-local Lucide icons beyond pb_import_kit's set (same mechanism).
...
export const TIC = {
    ...
    // FLEET P1 — the release stamp and the act of cutting one.
    ...
    // FLEET P2A — telling a customer something.
    ...
    // The shared kit carries `eyeOff` but not its opposite; this one is only
    // ever wanted on the platform's own screens, so it stays here rather than
    // being pushed into every customer's asset bundle for one preview heading.
    eye: '<path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>',
    // FLEET P2B — the waves, the windows and the controls on a rollout.
    ...
    // FLEET P3 — what is wrong right now, and the machine's own headroom.
...
export function tic(n, s = 16) {
    return markup(
        ...
        `stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${TIC[n] || TIC.globe}</svg>`
    );
}
