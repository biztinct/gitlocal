...
const I18N = {
  en: {
    ...
    live: {
      ...
      noDivision: "You do not have a division yet. Open Run Payroll once and you will be given one. Each demo account drives its own division's June run.",
      noRun: "Your division has no June run yet. Open Run Payroll and compute it. The wizard already has your division and your period selected.",
      ...
      stillDraft: "Still a draft. Submit the run for approval from the Pay Runs board.",
      notPastOfficer: "Nobody at the Payroll Officer gate has said yes yet.",
  ...
  vi: {
    ...
    live: {
      ...
      noDivision: "Bạn chưa có bộ phận nào. Hãy mở Chạy bảng lương một lần là bạn sẽ được gán. Mỗi tài khoản demo tự chạy đợt lương tháng 6 của bộ phận mình.",
      noRun: "Bộ phận của bạn chưa có đợt lương tháng 6. Hãy mở Chạy bảng lương và tính. Trình hướng dẫn đã chọn sẵn bộ phận và kỳ lương của bạn.",
      ...
      stillDraft: "Vẫn là bản nháp. Hãy trình đợt lương lên phê duyệt từ bảng Đợt tính lương.",
      notPastOfficer: "Chưa ai ở cổng Chuyên viên tính lương nói đồng ý.",
...
const GLOSSARY = {
  ...
  payrun: {
    ...
    def: B("One batch of payslips for one division and one month. It moves through the approval gates as a single thing: draft → Payroll Officer → {{hrTierName}} → {{gmTierName}} → done. A rejection at any gate CANCELS the whole batch, every payslip with it, and records who said no, when and why.",
           "Một lô phiếu lương của một bộ phận trong một tháng. Cả lô đi qua các cổng phê duyệt như một khối: Nháp → Chuyên viên tính lương → {{hrTierName}} → {{gmTierName}} → Hoàn tất. Bị từ chối ở bất kỳ cổng nào là HUỶ cả lô, kèm toàn bộ phiếu lương trong đó, và ghi lại ai từ chối, lúc nào và vì sao."),
  },
  period: {
  ...
  division: {
    ...
    def: B("A group of employees who are paid by the same rulebook. Choosing a division is how you choose which rules run, so it decides every number in the batch.",
           "Nhóm nhân viên được trả lương theo cùng một bộ quy tắc. Chọn bộ phận chính là chọn bộ quy tắc sẽ chạy, nên nó quyết định mọi con số trong lô."),
  },
  draft: {
    term: B("Draft", "Nháp"),
  ...
  /* -- who has to say yes ------------------------------------------------ */
  gate: {
    term: B("Approval gate", "Cổng phê duyệt"),
    aliases: { en: ["gate", "gates", "sign-off"], vi: ["cổng phê duyệt", "các cổng"] },
    def: B("One stop on the way to being paid, owned by one group of people. Nothing moves past a gate until somebody in that group says yes, and Payobook records who that was.",
           "Một chặng dừng trên đường tới lúc chi trả, thuộc về một nhóm người. Không gì đi qua được một cổng cho tới khi có người trong nhóm đó nói đồng ý, và Payobook ghi lại người đó là ai."),
  },
  tier: {
    term: B("Tier", "Cấp duyệt"),
    aliases: { en: ["tier", "tiers", "approval tier"], vi: ["cấp phê duyệt", "các cấp duyệt", "cấp duyệt", "cấp Chuyên viên tính lương", "cấp Chuyên viên", "cấp của bạn", "cấp của họ"] },
    def: B("The level of person who owns a gate. Tiers exist so that the person who prepares the pay is never the only person who signs for it.",
           "Cấp bậc người sở hữu một cổng. Các cấp duyệt tồn tại để người lập bảng lương không bao giờ là người duy nhất ký cho nó."),
  },
  approvalChain: {
    term: B("Approval chain", "Chuỗi phê duyệt"),
    aliases: { en: ["approval chain", "the chain", "pipeline"], vi: ["chuỗi phê duyệt", "chuỗi duyệt", "quy trình phê duyệt"] },
    def: B("The fixed order of gates a pay run travels through. Nobody can skip one, which is what makes the trail worth reading when somebody asks what happened.",
           "Thứ tự cố định các cổng mà một đợt lương phải đi qua. Không ai bỏ qua được cổng nào, và chính điều đó làm cho vết lưu đáng đọc khi có người hỏi chuyện gì đã xảy ra."),
  },
  payrollOfficer: {
    term: B("Payroll Officer", "Chuyên viên tính lương"),
    aliases: { en: ["officer", "payroll officers"], vi: ["chuyên viên lương"] },
    def: B("The first gate on a pay run, and usually the person who prepared it. It is the checkpoint that catches a mistake while fixing it still costs minutes.",
           "Cổng đầu tiên của một đợt lương, và thường là chính người đã lập nó. Đây là chốt bắt lỗi khi việc sửa còn chỉ tốn vài phút."),
  },
  rejection: {
    term: B("Rejection", "Từ chối"),
    aliases: { en: ["reject", "rejected", "rejecting"], vi: ["từ chối", "bị từ chối"] },
    def: B("Saying no at a gate. It CANCELS the whole batch and every payslip in it, with your name and your reason on the record.",
           "Nói không tại một cổng. Thao tác này HUỶ cả lô và toàn bộ phiếu lương trong đó, kèm tên bạn và lý do bạn ghi vào hồ sơ."),
  },
  flag: {
  ...
  proration: {
    ...
    aliases: { en: ["prorated", "prorate", "pro-rata", "part-month"], vi: ["theo ngày công", "tính theo tỷ lệ ngày công"] },
    def: B("Paying part of a month: days worked divided by the division's standard working days. It is a factor, not a discount — when an amount looks wrong, the factor is the thing to read first.",
           "Trả lương cho một phần của tháng: số ngày công thực tế chia cho ngày công chuẩn của bộ phận. Đây là một hệ số, không phải khoản giảm trừ — khi một con số trông sai, hệ số là thứ cần đọc trước tiên."),
  },
  joiner: {
  ...
  retro: {
    ...
    aliases: { en: ["retro", "retro line", "backdated"], vi: ["hồi tố", "dòng hồi tố", "truy lĩnh"] },
    def: B("Money owed for a month that is already closed, paid in the current run with the old month named on the line. It is how a closed month stays closed — you pay the difference instead of reopening a month somebody has already reported.",
           "Khoản tiền còn nợ của một tháng đã đóng, được chi trong đợt hiện tại và ghi rõ tháng gốc trên dòng đó. Đây là cách giữ cho một kỳ đã đóng vẫn luôn đóng — bạn trả phần chênh thay vì mở lại một tháng người khác đã báo cáo."),
  },
  ...
  configCode: {
    ...
    def: B("The short unique name of a formula configuration — what Run Payroll really picks when you choose a division. The shape is PREFIX_DIVISION_CYCLE, and the prefix belongs to the company: this practice company writes HOASEN_RETAIL_END, the Payobook demo world writes DEMO_RETAIL_END. Learn the shape, not one code.",
  ...
  salaryStructure: {
    ...
    def: B("Odoo's older way of computing a payslip, kept because payslips from before the move still point at one. Read them to understand history; write new pay logic in a formula configuration instead.",
           "Cách tính phiếu lương thế hệ trước của Odoo, được giữ lại vì các phiếu lương từ trước khi chuyển đổi vẫn trỏ tới chúng. Hãy đọc chúng để hiểu lịch sử; còn logic lương mới thì viết trong cấu hình công thức."),
  },
  connector: {
  ...
  confidenceScore: {
    ...
    def: B("How much of your file Payobook could read without having to guess. It is a percentage of ROWS, so a high score on a big file can still leave a handful of people to fix by hand.",
           "Phần dữ liệu trong tệp mà Payobook đọc được mà không phải đoán. Đây là tỷ lệ trên số DÒNG, nên một tệp lớn có điểm cao vẫn có thể còn vài người phải sửa tay."),
  },
  mapping: {
    ...
    stations: [
      {
        ...
        desc: B("Create a month of draft payslips for one division. A month of pay starts here.",
                "Tạo phiếu lương nháp của một tháng cho một bộ phận. Một tháng lương bắt đầu từ đây."),
        outline: {
          what: B("A wizard with four steps. You give it one division and one month. It works out a draft payslip for every employee who is eligible.",
                  "Một trình hướng dẫn bốn bước. Bạn đưa cho nó một bộ phận và một tháng. Nó tính phiếu lương nháp cho từng nhân viên đủ điều kiện."),
          why: B("Everything after this reads what this step produced — review, approval, the bank file. So a mistake here is a mistake in all of it.",
                 "Mọi việc phía sau đều đọc kết quả của bước này — soát xét, phê duyệt, tệp chi lương. Nên sai ở đây là sai ở tất cả."),
          when: B("Once the month's attendance and overtime are in and committed. Not before. Computing on an import nobody committed is the most common mistake of a first week.",
                  "Sau khi chấm công và tăng ca của tháng đã vào và đã ghi nhận. Không sớm hơn. Tính lương trên một đợt nhập chưa ai ghi nhận là lỗi phổ biến nhất của tuần đầu tiên."),
          prereq: B("The Payroll Officer group. A committed import for the month. And a formula configuration on the division you are running.",
                    "Nhóm quyền Chuyên viên tính lương. Một đợt nhập đã ghi nhận cho tháng đó. Và một cấu hình công thức trên bộ phận bạn định chạy."),
          mistakes: [
            B("Computing before the month's import is committed. The eligible count is the tell: 12 where you expected 48 means the data is not in yet.",
              "Tính lương khi đợt nhập của tháng chưa được ghi nhận. Số đủ điều kiện là dấu hiệu: thấy 12 trong khi bạn chờ 48 nghĩa là dữ liệu chưa vào."),
            B("Running the wrong cycle. Mid-cycle pays an advance. End-cycle settles the whole month. They are not two views of one thing.",
              "Chạy nhầm chu kỳ. Giữa kỳ là trả tạm ứng. Cuối kỳ là quyết toán cả tháng. Đó không phải hai cách nhìn của cùng một thứ."),
            B("Submitting for approval without opening the flagged payslips. A flag is a question the engine wants answered. Submitting answers it with silence.",
              "Trình phê duyệt mà chưa mở các phiếu bị gắn cờ. Cờ là câu hỏi hệ thống muốn được trả lời. Trình phê duyệt là trả lời nó bằng sự im lặng."),
          ],
        },
      },
      {
        ...
        desc: B("Every pay run on one board, in the column of the gate it is waiting at.",
                "Mọi đợt tính lương trên một bảng, nằm ở cột của đúng cổng nó đang chờ."),
        outline: {
          what: B("A board of every pay run. They are grouped by the gate each one is waiting at, and a card offers only what that gate allows.",
                  "Bảng chứa mọi đợt tính lương. Chúng được nhóm theo cổng mà từng đợt đang chờ, và mỗi thẻ chỉ mở ra những thao tác cổng đó cho phép."),
          why: B("Nothing is paid without the right signatures. This board is what makes that visible. It answers the question a payroll week really asks: what is stuck, and whose gate is it stuck at.",
                 "Không khoản nào được chi khi chưa đủ chữ ký. Bảng này làm điều đó hiện ra rõ ràng. Nó trả lời đúng câu hỏi mà tuần tính lương thực sự đặt ra: cái gì đang tắc, và tắc ở cổng của ai."),
          ...
          prereq: B("A run that has been computed. To approve one you also need the group that owns that gate.",
                    "Một đợt đã tính xong. Muốn phê duyệt thì bạn còn cần nhóm quyền sở hữu cổng đó."),
          mistakes: [
            B("Approving a run without opening its flagged payslips. The board shows you totals. The flags are on the payslips underneath.",
              "Phê duyệt một đợt mà chưa mở các phiếu bị gắn cờ. Bảng chỉ cho bạn thấy các số tổng. Cờ nằm trên từng phiếu lương bên dưới."),
            B("Rejecting without writing a reason. Payobook refuses an empty one, and a thin one is worse than none. The run is cancelled either way, and whoever rebuilds it has to guess what you saw.",
              "Từ chối mà không ghi lý do. Payobook không cho để trống, và một lý do hời hợt còn tệ hơn không có. Đằng nào đợt lương cũng bị huỷ, còn người dựng lại nó thì phải đoán bạn đã thấy gì."),
            B("Reading a status column as a month. The columns are approval stages. July and June can sit in the same one.",
              "Đọc cột trạng thái như thể đó là tháng. Các cột là bước phê duyệt. Tháng 7 và tháng 6 có thể cùng nằm trong một cột."),
          ],
        },
      },
      {
        ...
        outline: {
          what: B("Where you review the payslips inside one run. There is a list you can filter. For the person you pick, you also get the approval steps and the full breakdown of their pay.",
                  "Nơi bạn soát xét từng phiếu lương trong một đợt. Có một danh sách lọc được. Với người bạn chọn, bạn còn thấy các bước phê duyệt và bảng chi tiết lương đầy đủ."),
          why: B("A run total can be right while one payslip inside it is wrong. This is the only screen that shows the working behind a net figure. So this is where you catch an error before the money moves.",
                 "Tổng của một đợt có thể đúng trong khi một phiếu lương bên trong vẫn sai. Đây là màn hình duy nhất cho thấy phần tính toán phía sau con số thực nhận. Nên đây là nơi bạn bắt lỗi trước khi tiền chạy đi."),
          when: B("After computing and before submitting for approval. And again whenever somebody asks why their pay is what it is.",
                  "Sau khi tính và trước khi trình phê duyệt. Và bất cứ khi nào có người hỏi vì sao lương của họ lại như vậy."),
          prereq: B("A run that has been computed. You need no special group just to look.",
                    "Một đợt đã tính. Chỉ để xem thì bạn không cần nhóm quyền đặc biệt nào."),
          mistakes: [
            ...
            B("Clearing a flag by dismissing it. Clearing one should mean you understood it. The flag was the engine asking a question about that person.",
              "Xoá một cờ cảnh báo bằng cách bỏ qua nó. Xoá cờ đáng lẽ nghĩa là bạn đã hiểu nó. Cờ là hệ thống đang hỏi một câu về chính người đó."),
          ],
        },
      },
      {
        ...
        desc: B("Bring attendance, overtime and other inputs in. Preview them, score them and fix them before anything is committed.",
                "Đưa chấm công, tăng ca và các dữ liệu đầu vào khác vào. Xem trước, chấm điểm và sửa lỗi trước khi ghi nhận."),
        outline: {
          what: B("A guided flow with four steps: pick the source, review and match the columns, fix what is wrong, then commit. You also get the history of everything imported before.",
                  "Luồng có hướng dẫn gồm bốn bước: chọn nguồn, soát và khớp cột, sửa những chỗ sai, rồi ghi nhận. Bạn cũng có lịch sử mọi đợt đã nhập trước đó."),
          ...
          when: B("As soon as the month's inputs arrive. And always before you compute a run.",
                  "Ngay khi dữ liệu đầu vào của tháng về. Và luôn luôn trước khi bạn tính một đợt lương."),
          prereq: B("The Import group. A file, or a connected system to read from. And a formula configuration to map the columns onto.",
                    "Nhóm quyền Nhập dữ liệu. Một tệp, hoặc một hệ thống đã kết nối để đọc. Và một cấu hình công thức để ánh xạ các cột vào."),
          mistakes: [
            B("Committing a batch with rows still unresolved because the deadline is close. A row that did not match is a person who will be missing from the run.",
              "Ghi nhận một đợt còn dòng chưa xử lý vì sắp tới hạn. Một dòng không khớp là một người sẽ bị thiếu trong đợt lương."),
            B("Importing one month's data into another month. The batch is named for the month it targets, so read that name before you commit.",
              "Nhập dữ liệu của tháng này vào tháng khác. Đợt nhập được đặt tên theo tháng mà nó nhắm tới, nên hãy đọc tên đó trước khi ghi nhận."),
      ...
      },
      {
        ...
        desc: B("Settle somebody who is leaving: last salary, unused leave, anything owed. One closing statement.",
                "Quyết toán cho người thôi việc: lương cuối, phép chưa dùng, mọi khoản còn nợ. Một bảng chốt duy nhất."),
        outline: {
          what: B("The workspace for people who are leaving. It lists what each of them is still owed, and whether it has been paid.",
                  "Không gian làm việc dành cho người sắp nghỉ. Nó liệt kê từng người còn được nhận gì, và đã chi hay chưa."),
          why: B("This payment has a legal deadline and one chance to be right. It is also the payment most likely to be argued about, because it is the last one.",
                 "Khoản chi này có thời hạn pháp lý và chỉ có một lần làm đúng. Đây cũng là khoản dễ bị tranh cãi nhất, vì là khoản cuối cùng."),
          when: B("As soon as the leaving date is confirmed. Not on the day you run payroll, which is usually too late.",
                  "Ngay khi ngày nghỉ được xác nhận. Không phải vào ngày bạn chạy lương, lúc đó thường đã muộn."),
          prereq: B("A contract end date on the record, and an up-to-date leave balance for that person.",
                    "Ngày kết thúc hợp đồng đã ghi trên hồ sơ, và số dư phép cập nhật của người đó."),
          mistakes: [
            B("Leaving the person in the normal monthly run as well. They are then paid twice. Getting an overpayment back from somebody who has left is a legal problem, not a payroll one.",
              "Vẫn để người đó trong đợt lương tháng bình thường. Họ sẽ được trả hai lần. Đòi lại khoản trả thừa từ người đã nghỉ là chuyện pháp lý, không còn là chuyện tính lương."),
            B("Settling before the last month's attendance is in. The final salary is a prorated month, so it needs the same inputs as any other month.",
              "Quyết toán khi chấm công tháng cuối chưa về. Lương cuối là một tháng tính theo ngày công, nên cần đúng những dữ liệu như mọi tháng khác."),
          ],
        },
      },
      {
        ...
        desc: B("See exactly how a part-month salary was worked out. Day by day, with no black box.",
                "Xem chính xác lương tháng lẻ ngày được tính ra sao. Từng ngày một, không có hộp đen nào."),
        outline: {
          what: B("A record of every prorated amount in a run — joiners, leavers and unpaid leave. Each row shows days worked, standard days, and the factor between them.",
                  "Bản ghi mọi khoản tính theo ngày công trong một đợt — người mới vào, người thôi việc và nghỉ không lương. Mỗi dòng hiện ngày công thực tế, ngày công chuẩn, và hệ số giữa hai con số đó."),
          why: B("Proration is what employees ask about most, and it is the hardest thing to answer from memory. This screen is the evidence, written down before anybody asks for it.",
                 "Tính theo ngày công là thứ nhân viên hỏi nhiều nhất, và cũng là thứ khó trả lời nhất nếu chỉ dựa vào trí nhớ. Màn hình này là bằng chứng, đã ghi sẵn trước khi có ai hỏi tới."),
          when: B("When a payslip looks too small and you need to show why. And as a spot check on any run with people joining or leaving mid-month.",
                  "Khi một phiếu lương trông thiếu và bạn cần chứng minh vì sao. Và như một bước kiểm tra ngẫu nhiên cho mọi đợt có người vào hoặc nghỉ giữa tháng."),
          prereq: B("A run that has been computed, with at least one joiner, leaver or unpaid-leave case in it.",
                    "Một đợt đã tính, trong đó có ít nhất một trường hợp mới vào, thôi việc hoặc nghỉ không lương."),
          mistakes: [
            B("Assuming calendar days when the division's rulebook uses standard working days. Twenty-two is not thirty, and the difference is somebody's salary.",
              "Cho rằng đó là ngày dương lịch trong khi bộ quy tắc của bộ phận dùng ngày công chuẩn. 22 không phải 30, và phần chênh là lương của một người."),
            B("Reading the money before the factor. If the factor is right and the amount still looks wrong, the problem is further back: the contract, or the import.",
              "Đọc số tiền trước khi đọc hệ số. Nếu hệ số đúng mà số tiền vẫn có vẻ sai thì vấn đề nằm ở phía trước đó: hợp đồng, hoặc đợt nhập liệu."),
          ],
        },
      },
      {
        ...
        desc: B("Backdated raises and corrections, paid cleanly in a later run, with a full trail.",
                "Tăng lương lùi ngày và các hiệu chỉnh, trả gọn trong một đợt sau, có vết lưu đầy đủ."),
        outline: {
          what: B("A ledger of money still owed from months that are already closed. It is paid in the current run, and every line names the month it came from.",
                  "Sổ ghi những khoản tiền còn nợ từ các tháng đã đóng. Chúng được chi trong đợt hiện tại, và mỗi dòng đều nêu rõ tháng gốc."),
          why: B("It keeps closed months closed. Reopening an approved run to fix the past breaks every report already made from it. A retro line pays the difference without doing that.",
                 "Nó giữ cho các tháng đã đóng luôn đóng. Mở lại một đợt đã duyệt để sửa quá khứ làm hỏng mọi báo cáo đã lập từ đợt đó. Dòng hồi tố trả phần chênh mà không phải làm vậy."),
          when: B("Backdated raises. Allowances missed in an earlier month. And any correction you find after the run it belongs to was approved.",
                  "Tăng lương lùi ngày. Phụ cấp bị sót ở tháng trước. Và mọi hiệu chỉnh bạn phát hiện sau khi đợt gốc đã được phê duyệt."),
          prereq: B("The original run is done, and the correction has a document behind it. A retro line with no reason is a payment nobody can explain.",
                    "Đợt gốc đã Hoàn tất, và hiệu chỉnh có chứng từ đi kèm. Một dòng hồi tố không có lý do là khoản chi không ai giải thích được."),
          mistakes: [
            B("Editing the old approved payslip instead of writing a retro line. That month has already been reported, so you are changing a number somebody else has filed.",
              "Sửa phiếu lương cũ đã duyệt thay vì viết một dòng hồi tố. Tháng đó đã được báo cáo, nên bạn đang sửa một con số người khác đã nộp đi."),
            B("Leaving the source month blank. Without it the line is money paid for no stated reason, which is the first thing an audit asks about.",
              "Bỏ trống tháng gốc. Không có nó, dòng này là khoản chi không nêu lý do, và đó là thứ kiểm toán hỏi đầu tiên."),
          ],
        },
      },
    ],
    ...
    stations: [
      {
        ...
        outline: {
          what: B("A workshop for each division's formula configuration. You see the components, the formula behind each one, what it depends on, and a preview run against a real employee.",
                  "Xưởng làm việc cho cấu hình công thức của từng bộ phận. Bạn thấy các thành phần, công thức đằng sau mỗi thành phần, nó phụ thuộc vào gì, và bản xem trước chạy trên một nhân viên thật."),
          ...
          prereq: B("The Payroll Officer group to read it, and a manager tier to change it. You also want a sample employee to preview against.",
                    "Nhóm quyền Chuyên viên tính lương để xem, và cấp quản lý để sửa. Bạn cũng nên có một nhân viên mẫu để xem trước."),
          mistakes: [
            B("Editing a live configuration without previewing first. The preview costs seconds. A wrong component costs every future payslip in that division.",
            ...
            B("Typing a statutory rate straight into a formula. Rates belong on the insurance policy. A hand-typed 8% is one nobody will remember to change.",
              "Gõ thẳng một tỷ lệ luật định vào công thức. Tỷ lệ thuộc về chính sách bảo hiểm. Một con số 8% gõ tay là con số không ai nhớ để sửa."),
          ],
        },
      },
      {
        ...
        desc: B("The old rule sets. They are kept because old payslips still point at them, not because new pay logic belongs here.",
                "Các bộ quy tắc cũ. Chúng được giữ lại vì phiếu lương cũ vẫn trỏ tới chúng, không phải vì logic lương mới nên nằm ở đây."),
        outline: {
          what: B("Odoo salary structures and their rules. This is the older way of computing a payslip, and it still holds the logic that produced your historical payslips.",
                  "Cấu trúc lương và các quy tắc của Odoo. Đây là cách tính phiếu lương thế hệ trước, và nó vẫn giữ phần logic đã tạo ra các phiếu lương lịch sử của bạn."),
          why: B("You meet one the first time you open a payslip from before the move. Knowing what it is decides whether you read that history or doubt it.",
                 "Bạn gặp chúng ngay lần đầu mở một phiếu lương từ trước khi chuyển đổi. Hiểu chúng là gì sẽ quyết định bạn đọc được lịch sử đó hay hoài nghi nó."),
          when: B("When you read an old payslip. And when you move a division onto a formula configuration — the structure is what you are moving away from.",
                  "Khi bạn đọc một phiếu lương cũ. Và khi bạn chuyển một bộ phận sang cấu hình công thức — cấu trúc chính là thứ bạn đang rời khỏi."),
          prereq: B("A manager tier, and the basics of the Formula Engine. Without those it is hard to see what the two are for.",
                    "Cấp quản lý, và phần cơ bản của Công thức lương. Thiếu hai thứ đó thì khó thấy chúng dùng để làm gì."),
          mistakes: [
            B("Building new pay logic here because it feels familiar. The Formula Engine cannot see a rule added to a structure, so the division it was meant for never gets it.",
              "Xây logic lương mới ở đây vì thấy quen tay. Công thức lương không nhìn thấy một quy tắc thêm vào cấu trúc, nên bộ phận cần nó sẽ chẳng bao giờ nhận được."),
            B("Deleting a structure because nobody uses it today. Payslips from three years ago still point at it, and a report over those months needs it to still be there.",
              "Xoá một cấu trúc vì hôm nay không ai dùng. Phiếu lương của ba năm trước vẫn trỏ tới nó, và một báo cáo trên các tháng đó vẫn cần nó tồn tại."),
          ],
        },
      },
      {
        ...
        outline: {
          ...
          prereq: B("A manager tier, and the decree open in front of you. This screen records a decision that was made somewhere else.",
                    "Cấp quản lý, và nghị định đang mở trước mặt bạn. Màn hình này ghi lại một quyết định đã được ra ở nơi khác."),
          mistakes: [
      ...
      },
      {
        ...
        desc: B("Connectors that pull attendance and HR data in on their own, mapped field by field, with a sync history.",
                "Các đầu nối tự kéo dữ liệu chấm công và nhân sự về, ánh xạ theo từng trường, kèm lịch sử đồng bộ."),
        outline: {
          what: B("Saved links to the systems payroll data arrives from: an HR system, a time clock, the bank. Each one shows its field mapping and when it last synced.",
                  "Các kết nối đã lưu tới những hệ thống mà dữ liệu lương đi vào từ đó: hệ thống nhân sự, máy chấm công, ngân hàng. Mỗi kết nối hiện ánh xạ trường của nó và lần đồng bộ gần nhất."),
          why: B("Data that arrives by itself is data nobody retyped, and a retyped row is where most wrong payslips start. The sync history is the part that matters: a connector that stopped looks exactly like one that is working.",
                 "Dữ liệu tự về là dữ liệu không ai phải gõ lại, mà một dòng gõ tay là khởi đầu của phần lớn phiếu lương sai. Phần quan trọng là lịch sử đồng bộ: một đầu nối đã ngừng chạy trông y hệt một đầu nối đang chạy tốt."),
          when: B("Once when you set the system up. Again whenever a source system changes its fields. And as a two-minute check at the start of every payroll week.",
                  "Một lần khi bạn thiết lập hệ thống. Làm lại mỗi khi hệ thống nguồn đổi trường dữ liệu. Và như một bước kiểm tra hai phút vào đầu mỗi tuần tính lương."),
          prereq: B("The Integration user group, and the login details for the source system.",
                    "Nhóm quyền Người dùng tích hợp, và thông tin đăng nhập của hệ thống nguồn."),
          mistakes: [
            B("Letting a failed sync go unnoticed until payroll week. The staged-record count is the tell: rows sitting in staging are a month of attendance that never became inputs.",
    ...
    stations: [
      {
        ...
        outline: {
          what: B("Your starting screen. It says who you are, which run is live and four numbers that describe the company. Below that, a card for each part of Payobook worth opening today.",
                  "Màn hình bạn bắt đầu. Nó cho biết bạn là ai, đợt lương nào đang chạy, và bốn con số mô tả cả công ty. Bên dưới là một thẻ cho mỗi phần của Payobook đáng mở hôm nay."),
          why: B("It is the only screen that answers \"how is payroll doing right now\" without you choosing a filter first. The tiles report. The buttons and the run rows beside them are the doors into the screens those figures came from.",
                 "Đây là màn hình duy nhất trả lời được \"công việc lương lúc này ra sao\" mà bạn không phải chọn bộ lọc trước. Các ô chỉ số chỉ báo cáo. Còn các nút và các dòng đợt lương bên cạnh mới là cửa dẫn vào những màn hình đã sinh ra các con số ấy."),
          ...
          prereq: B("Nothing at all. This is the one screen everybody can open, and it is deliberately the least gated in Payobook.",
                    "Không cần gì cả. Đây là màn hình ai cũng mở được, và nó cố ý là nơi ít bị chặn quyền nhất trong Payobook."),
          mistakes: [
            B("Reading a dashboard figure as today's work. Headcount and monthly payroll describe the company. Only \"pending approval\" is a queue, and only part of that queue is yours.",
              "Đọc một con số trên bảng điều khiển như thể đó là việc của hôm nay. Sĩ số và chi phí lương tháng mô tả cả công ty. Chỉ \"chờ phê duyệt\" mới là hàng đợi, và chỉ một phần hàng đợi đó là của bạn."),
            B("Treating it as a report and copying the numbers out. The KPI tiles only report. The buttons and the run rows are what open the screen a figure came from. A figure copied away from its working loses the part somebody will ask about.",
              "Coi nó như một báo cáo rồi chép số ra ngoài. Các ô chỉ số chỉ báo cáo. Chính các nút và các dòng đợt lương mới mở ra màn hình đã sinh ra con số. Một con số bị chép rời khỏi phần tính toán sẽ mất đúng phần sau này có người hỏi tới."),
      ...
      },
      {
        ...
        outline: {
          what: B("Three lanes: Officer review, {{hrTierName}} and {{gmTierName}}. Each submitted run is a card. The header adds up the money at stake, and a list below shows what has already been decided.",
                  "Ba làn: Chuyên viên soát, {{hrTierName}} và {{gmTierName}}. Mỗi đợt đã trình là một thẻ. Phần đầu cộng lại số tiền đang treo, còn danh sách bên dưới cho thấy những gì đã được quyết."),
          why: B("The Pay Runs board shows every run there is. This screen shows only the ones a person still has to answer for. The number in its header is the part of that queue that will not move until you move it.",
                 "Bảng Đợt tính lương hiển thị mọi đợt đang có. Màn hình này chỉ hiển thị những đợt vẫn cần một con người chịu trách nhiệm. Con số ở tiêu đề chính là phần hàng đợi sẽ không nhúc nhích cho tới khi bạn động vào."),
          when: B("Every morning of payroll week. And before you tell anybody a run is stuck — the lane it sits in names who is holding it.",
          ...
          prereq: B("One of the payroll approval groups. Without one, this screen is not in your menu at all — which is the honest way of saying you cannot approve.",
                    "Một trong các nhóm quyền phê duyệt lương. Không có nhóm nào thì màn hình này không xuất hiện trong menu của bạn — đó là cách nói thẳng thắn rằng bạn chưa phê duyệt được."),
          mistakes: [
            B("Approving from the lane without opening the run. The card shows a total and a payslip count. The flags are on the payslips, and the total is exactly where a wrong payslip hides.",
            ...
            B("Rejecting instead of asking. A rejection CANCELS the whole batch, with your name on the record, and only the {{gmTierName}} tier can turn it back into a draft. A question in the corridor costs nobody two days.",
              "Từ chối thay vì hỏi. Một lần từ chối HUỶ cả lô, kèm tên bạn trong hồ sơ, và chỉ cấp {{gmTierName}} mới đưa nó về lại bản nháp được. Một câu hỏi ngoài hành lang thì không làm ai mất hai ngày."),
          ],
        },
      },
    ],
    ...
    stations: [
      {
        ...
        outline: {
          what: B("A board of analytics over the runs this company has. The headline net comes from the LATEST run, whatever state it is in. Below that: the cost story over three, six or twelve months, and a department leaderboard read from the latest run that is DONE. Then the statutory split, a workforce pulse and the snapshots panel.",
                  "Bảng phân tích trên các đợt lương của công ty. Con số thực chi nổi bật lấy từ đợt GẦN NHẤT, bất kể nó đang ở trạng thái nào. Bên dưới là diễn biến chi phí theo ba, sáu hay mười hai tháng, và bảng xếp hạng bộ phận đọc từ đợt ĐÃ HOÀN TẤT gần nhất. Rồi tới cơ cấu các khoản đóng bắt buộc, nhịp nhân sự và bảng ảnh chụp số liệu."),
          why: B("It answers the questions a payroll month asks every time, and it is fast because it reads the stored roll-ups instead of adding payslip lines again. One thing to know: the headline and the leaderboard do not have the same scope. The headline shows the latest run and prints its state beside it. The leaderboard waits for a run to be done. Read the state chip before you quote the number.",
                 "Nó trả lời những câu hỏi mà tháng lương nào cũng đặt ra, và nó nhanh vì đọc các số tổng đã lưu thay vì cộng lại từng dòng phiếu lương. Có một điều cần biết: phần đầu và bảng xếp hạng không cùng phạm vi. Phần đầu hiển thị đợt gần nhất và in kèm trạng thái của chính nó. Bảng xếp hạng thì chờ tới khi có đợt đã hoàn tất. Hãy đọc chip trạng thái trước khi trích con số."),
          ...
          prereq: B("An analytics group, and at least two completed runs. A trend needs a second point before it is a trend.",
                    "Nhóm quyền phân tích, và ít nhất hai đợt lương đã hoàn tất. Một xu hướng cần điểm thứ hai mới thành xu hướng."),
          mistakes: [
            B("Comparing one total against another total. Headcount moves between months, so the honest comparison is cost per head. One extra employee explains most of what looks like a rise.",
              "Đem một con số tổng so với một con số tổng khác. Sĩ số thay đổi giữa các tháng, nên phép so trung thực là chi phí bình quân đầu người. Thêm một nhân viên là đủ giải thích phần lớn cái vẻ \"tăng\" đó."),
            B("Reading a board figure without its window. Three months and twelve months tell different stories about the same company, and the chip that decides which is easy to leave where the last person left it.",
            ...
            B("Quoting the headline without its state chip. The headline is the LATEST run in any state. A draft nobody has reviewed appears here the moment it is computed. It reads exactly like a paid month unless you look at the chip.",
              "Trích con số nổi bật mà bỏ qua chip trạng thái. Phần đầu lấy đợt GẦN NHẤT ở bất kỳ trạng thái nào. Một đợt còn Nháp chưa ai soát vẫn xuất hiện ở đây ngay khi vừa tính xong. Nhìn vào không khác gì một tháng đã chi, trừ khi bạn nhìn cái chip."),
          ],
        },
      },
      {
        ...
        outline: {
          what: B("A question builder over the same payroll facts. You pick one measure, one breakdown and one comparison. Filters appear as tags above the result, and you can pull any of them off.",
                  "Công cụ đặt câu hỏi trên cùng dữ liệu lương. Bạn chọn một chỉ tiêu, một chiều tách và một chiều so sánh. Các bộ lọc hiện thành thẻ phía trên kết quả, và bạn gỡ thẻ nào cũng được."),
          why: B("Boards answer the questions somebody expected, and real questions are rarely expected. This is where \"net pay by division, only the divisions that ran mid-cycle\" gets answered without anybody exporting a spreadsheet. It also reconciles with the payslips, which a spreadsheet stops doing the moment it is saved. And it is the only analytics screen that explains a MOVEMENT rather than reporting one.",
                 "Bảng phân tích trả lời những câu hỏi có người đã lường trước, còn câu hỏi thật thì hiếm khi được lường trước. Đây là nơi \"thực nhận theo bộ phận, chỉ các bộ phận chạy giữa kỳ\" được trả lời mà không ai phải xuất bảng tính. Nó cũng luôn khớp với phiếu lương, điều mà một bảng tính hết làm được ngay khi vừa lưu. Và đây là màn hình phân tích duy nhất giải thích được một BIẾN ĐỘNG chứ không chỉ báo cáo nó."),
          when: B("When the board has answered everything it can and the question is still open. Or when somebody needs a figure broken down a way nobody built a screen for.",
                  "Khi bảng phân tích đã trả lời hết những gì nó có thể mà câu hỏi vẫn còn đó. Hoặc khi ai đó cần một con số tách theo cách chưa có màn hình nào dựng sẵn."),
          prereq: B("An analytics group and a computed run. Knowing which measure you actually want helps more than any filter.",
                    "Nhóm quyền phân tích và một đợt lương đã tính. Biết rõ mình thật sự cần chỉ tiêu nào còn hữu ích hơn mọi bộ lọc."),
          mistakes: [
            B("Quoting a figure without its filters. The tags above the result are part of the answer. The same measure with one tag removed is a different number, and it looks identical pasted into an email.",
              "Trích một con số mà bỏ quên bộ lọc của nó. Các thẻ phía trên kết quả là một phần của câu trả lời. Cùng chỉ tiêu đó, gỡ một thẻ đi là một con số khác, mà dán vào email thì trông y hệt nhau."),
            B("Stopping at the table. The answer is usually in the waterfall and the drill. The table tells you a division moved; those two tell you which component moved and which employees carried it.",
              "Dừng lại ở cái bảng. Câu trả lời thường nằm ở biểu đồ phân rã và ở bước đi sâu. Cái bảng chỉ cho biết một bộ phận có biến động; hai thứ kia mới cho biết thành phần nào biến động và những nhân viên nào gánh phần đó."),
          ],
        },
      },
      {
        ...
        outline: {
          what: B("How many people were paid each month, who joined, who left, cost per head, attendance exceptions and overtime. All of it is read off the payroll that actually ran.",
                  "Mỗi tháng có bao nhiêu người được trả lương, ai vào, ai nghỉ, chi phí bình quân đầu người, ngoại lệ chấm công và tăng ca. Tất cả đọc từ chính các đợt lương đã chạy."),
          why: B("\"People paid\" is not \"people employed\", and the gap between those two numbers is where a missing payslip lives. A month with flat headcount and a jump in cost per head is a different problem from one where both move.",
                 "\"Được trả lương\" không đồng nghĩa với \"đang làm việc\", và khoảng chênh giữa hai con số ấy chính là chỗ một phiếu lương bị thiếu đang nằm. Một tháng sĩ số đứng yên mà chi phí bình quân nhảy vọt là vấn đề khác hẳn tháng cả hai cùng thay đổi."),
          when: B("At month end, beside Insights. And whenever a run's headcount does not match what you expected.",
                  "Vào cuối tháng, đọc cùng với Phân tích. Và bất cứ khi nào sĩ số của một đợt lương không khớp với điều bạn chờ đợi."),
          prereq: B("An analytics group, and at least two runs to compare.",
                    "Nhóm quyền phân tích, và ít nhất hai đợt lương để so sánh."),
          mistakes: [
            B("Reading the headcount line as a hiring chart. It counts people who were PAID, so a step in it can just as easily be a run that left somebody out.",
            ...
            B("Treating attendance exceptions as somebody else's problem. Each one becomes an input, and the ones nobody resolved are the flags on next month's payslips.",
              "Coi ngoại lệ chấm công là chuyện của người khác. Mỗi ngoại lệ đều thành một dữ liệu đầu vào, và những ngoại lệ không ai xử lý chính là các cờ cảnh báo trên phiếu lương tháng sau."),
          ],
        },
      },
    ],
...
const MORPHS = {
  ...
  runJuneJuly: {
    ...
    after: {
      ...
      delta: B("16,370,000 ₫ more, which is 2.7% on one extra employee. That is a comfortable-looking number. Comfortable-looking numbers are where one wrong payslip hides best: Hùng's 3,100,000 ₫ of extra overtime sits inside this total, and nothing about the total says so.",
               "Nhiều hơn 16.370.000 ₫, tức 2,7% với thêm một nhân viên. Đó là con số trông rất dễ chịu. Những con số dễ chịu chính là nơi một phiếu lương sai ẩn mình tốt nhất: phần tăng ca thêm 3.100.000 ₫ của Hùng nằm gọn trong con số tổng này, và bản thân con số tổng không hề nói ra điều đó."),
    },
  },
};
  ...
  L1: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("Payobook prints which configuration will run and how many employees are eligible. That count is your smoke alarm. If it says 12 where you expected 48, <b>stop</b>. The month's attendance import has almost certainly not been committed. Computing now would produce 12 real-looking payslips and 36 missing ones.",
                "Payobook in ra cấu hình nào sẽ chạy và bao nhiêu nhân viên đủ điều kiện. Con số đó là chuông báo khói của bạn. Nếu nó hiện 12 trong khi bạn chờ 48, hãy <b>dừng lại</b>. Gần như chắc chắn chấm công của tháng chưa được ghi nhận. Tính lúc này sẽ tạo ra 12 phiếu trông rất thật và 36 phiếu thiếu."),
      },
      {
      ...
      },
      {
        ...
        body: B("The run is now in <b>draft</b> on the Pay Runs board. From here it travels through the Payroll Officer tier, {{hrTierName}}, {{gmTierName}} and then done. Each gate belongs to one group, and nobody can skip one. A rejection at any of them <b>cancels</b> the run and every payslip in it, with a written reason on the record.",
                "Đợt lương giờ ở trạng thái <b>Nháp</b> trên bảng Đợt tính lương. Từ đây nó đi qua cấp Chuyên viên tính lương, {{hrTierName}}, {{gmTierName}} rồi Hoàn tất. Mỗi cổng thuộc về một nhóm quyền, và không ai bỏ qua được cổng nào. Bị từ chối ở bất kỳ cổng nào cũng <b>huỷ</b> đợt lương và toàn bộ phiếu lương trong đó, kèm lý do ghi bằng văn bản vào hồ sơ."),
      ...
      options: [
        {
          ...
          correct: false,
          explanation: B("Let's rethink that. Editing the result breaks the trail. The payslip no longer agrees with the data behind it. The next recompute quietly undoes your fix, so the mistake comes back without anybody touching it.",
                         "Hãy nghĩ lại một chút. Sửa kết quả làm đứt vết dữ liệu. Phiếu lương không còn khớp với dữ liệu phía sau. Lần tính lại kế tiếp âm thầm xoá sửa đổi của bạn, nên lỗi quay lại mà không ai chạm vào."),
        },
        {
          text: B("Fix the overtime input, then recompute the draft", "Sửa dữ liệu tăng ca, rồi tính lại bản nháp"),
          correct: true,
          explanation: B("Exactly. Drafts exist for this. Correct the input, then recompute. Every line that depends on it — gross, thuế TNCN, net — corrects itself, because all of them were worked out from that input.",
                         "Chính xác. Bản nháp sinh ra để làm việc đó. Sửa đầu vào, rồi tính lại. Mọi dòng phụ thuộc vào nó — tổng thu nhập, thuế TNCN, thực nhận — tự đúng theo, vì tất cả đều được tính ra từ đầu vào đó."),
        },
        {
  ...
  L2: {
    ...
    steps: [
      {
        ...
        body: B("Total runs, in pipeline, <b>awaiting your approval</b>, completed, net paid. Four of those describe the department. The third one describes you. It is the work that will not move until you move it. Read it first, every morning of payroll week.",
                "Tổng số đợt, đang trong quy trình, <b>chờ bạn phê duyệt</b>, đã hoàn tất, đã chi. Bốn con số mô tả cả bộ phận. Con số thứ ba mô tả bạn. Đó là phần việc sẽ không nhúc nhích cho tới khi bạn động vào. Hãy đọc nó đầu tiên, mỗi sáng trong tuần tính lương."),
      },
      {
        ...
        body: B("This is the single most common misreading of this screen. A column is an <b>approval stage</b>. July and June can sit side by side in the same one. A run moves left to right by being approved, never by being edited. Use the date chips below when you want one month.",
                "Đây là cách đọc sai phổ biến nhất của màn hình này. Một cột là một <b>bước phê duyệt</b>. Tháng 7 và tháng 6 có thể nằm cạnh nhau trong cùng một cột. Một đợt đi từ trái sang phải bằng việc được phê duyệt, không bao giờ bằng việc bị sửa. Muốn lọc theo một tháng, hãy dùng các chip ngày ở dưới."),
      },
      {
        screen: "payruns", anchor: "rep-pipeline",
      ...
      },
      {
        ...
        body: B("The buttons on a card are decided by the record's own gate fields and your groups — not by the card. If you cannot see Approve here, you do not hold that tier, and pressing harder will not help. That is also why the same run shows different buttons to different people.",
                "Các nút trên thẻ do chính các trường kiểm soát cổng của bản ghi và nhóm quyền của bạn quyết định — không phải do thẻ. Nếu bạn không thấy nút Phê duyệt ở đây thì bạn không giữ cấp đó, và bấm mạnh hơn cũng không giúp được. Đó cũng là lý do cùng một đợt lương lại hiện các nút khác nhau với những người khác nhau."),
      },
      {
        ...
        body: B("Rejecting <b>cancels</b> the run. It does not send it back. The run's own state becomes Rejected, and every payslip in it is cancelled at the same time. Three things are recorded: who rejected it, when, and <b>why, in writing</b>. That written reason is the whole value of the rejection. Without it, the officer rebuilding the run is guessing at what you saw.",
                "Từ chối là <b>huỷ</b> đợt lương. Nó không trả lại đợt lương cho ai cả. Trạng thái của chính đợt chuyển thành Đã từ chối, và mọi phiếu lương trong đó cùng bị huỷ một lượt. Hệ thống ghi lại ba điều: ai từ chối, vào lúc nào, và <b>vì sao, bằng văn bản</b>. Chính lý do bằng văn bản đó mới là giá trị của việc từ chối. Không có nó, chuyên viên dựng lại đợt lương chỉ còn cách đoán xem bạn đã thấy gì."),
        consequence: B("Affects the whole run, not one payslip: all 48 are cancelled together and the run reads Rejected. Reversible: <b>not by you</b>. Turning a run back into a draft is a {{gmTierName}} action. Until somebody at that tier does it, the officer has nothing to recompute. Nothing was paid. Verify first: that the problem really is the whole run, and not one payslip you could have discussed.",
                       "Ảnh hưởng cả đợt lương, không phải một phiếu: cả 48 phiếu cùng bị huỷ và đợt chuyển sang trạng thái Đã từ chối. Hoàn tác: <b>không phải bởi bạn</b>. Đưa một đợt về lại bản nháp là thao tác của cấp {{gmTierName}}. Tới khi có người ở cấp đó làm việc ấy, chuyên viên chẳng có gì để tính lại. Chưa có gì được chi. Kiểm tra trước: vấn đề có thực sự nằm ở cả đợt không, hay chỉ là một phiếu bạn có thể trao đổi riêng."),
      },
      {
    ...
    quiz: {
      ...
      options: [
        ...
        {
          text: B("Ask the person who holds that tier — the column names them", "Hỏi người giữ cấp đó — cột đã chỉ đích danh"),
          correct: true,
          explanation: B("Yes. The column a run is sitting in tells you who can move it. Chasing the right person is the only thing that shortens the wait. It is also the fastest way to find out whether they are stuck on something you can fix.",
                         "Đúng vậy. Cột mà đợt lương đang nằm cho bạn biết ai có thể đẩy nó đi. Hỏi đúng người là cách duy nhất rút ngắn thời gian chờ. Đó cũng là cách nhanh nhất để biết họ đang vướng điều gì mà bạn gỡ được."),
        },
        {
          text: B("Reject it yourself and start a clean run", "Tự từ chối rồi chạy lại một đợt sạch"),
          correct: false,
          explanation: B("Let's rethink that. A rejection is testimony about a problem you found, and it CANCELS the run. Using it to hurry a deadline puts an untrue reason on the record. It throws away two days of somebody's review. And the batch then needs a {{gmTierName}} reset before anybody can recompute it.",
                         "Hãy nghĩ lại một chút. Từ chối là lời chứng về một vấn đề bạn phát hiện, và nó HUỶ đợt lương. Dùng nó để chạy cho kịp hạn là ghi vào hồ sơ một lý do không đúng sự thật. Nó vứt bỏ hai ngày soát xét của người khác. Và lô đó phải chờ cấp {{gmTierName}} mở lại thì mới có ai tính lại được."),
        },
      ],
    },
  },
  ...
  L3: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("BHXH, BHYT and BHTN are charged on the <b>insurance base</b> — for Mai, 12,000,000 ₫, the salary in her contract. Not on gross. That is why 1,500,000 ₫ of overtime moved her tax and did not move her insurance by a single đồng. It answers most variance questions before anybody asks them.",
                "BHXH, BHYT và BHTN tính trên <b>mức lương đóng bảo hiểm</b> — với Mai là 12.000.000 ₫, tức mức lương ghi trong hợp đồng. Không tính trên tổng thu nhập. Vì vậy 1.500.000 ₫ tăng ca làm thay đổi thuế của cô ấy nhưng không làm bảo hiểm nhúc nhích một đồng. Điều đó trả lời hầu hết thắc mắc về biến động trước cả khi có ai hỏi."),
      ...
      options: [
        {
          ...
          correct: false,
          explanation: B("Let's rethink that. It is working exactly as the law describes. Contributions are charged on the insurance base, and overtime is not part of that base. Reporting it would send somebody looking for a bug that is not there.",
                         "Hãy nghĩ lại một chút. Nó đang chạy đúng như luật quy định. Bảo hiểm tính trên mức lương đóng bảo hiểm, và tăng ca không nằm trong mức đó. Báo lỗi sẽ khiến ai đó đi tìm một lỗi không tồn tại."),
        },
        {
          ...
          correct: true,
          explanation: B("Yes. Insurance follows the contract, tax follows the month. This one fact explains most of the variance questions a payroll desk gets. Being able to say it plainly is worth more than any report.",
                         "Đúng vậy. Bảo hiểm bám theo hợp đồng, thuế bám theo tháng. Một điều đó thôi đã giải thích được phần lớn thắc mắc về biến động mà bộ phận lương nhận được. Nói được nó rõ ràng còn giá trị hơn bất kỳ báo cáo nào."),
        },
        {
          ...
          correct: false,
          explanation: B("Let's rethink that. The insurance base is a figure declared in the contract. Payroll does not raise it to follow a busy month. Changing it is an HR and legal decision, with consequences well beyond this payslip.",
                         "Hãy nghĩ lại một chút. Mức lương đóng bảo hiểm là con số khai báo theo hợp đồng. Bộ phận lương không nâng nó lên chỉ vì một tháng bận rộn. Thay đổi nó là quyết định của nhân sự và pháp chế, với hệ quả vượt xa phiếu lương này."),
        },
      ],
    },
  },
  ...
  L4: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("It is tempting. The payslip is right in front of you and the number is obviously wrong. But editing here corrects the result and leaves the input untouched. The next recompute brings the mistake straight back. Now the payslip and the timesheet disagree, which is worse than the original error.",
                "Rất dễ bị cám dỗ. Phiếu lương đang ngay trước mặt và con số rõ ràng là sai. Nhưng sửa ở đây chỉ chỉnh kết quả và để nguyên đầu vào. Lần tính lại kế tiếp mang lỗi quay lại ngay. Giờ thì phiếu lương và bảng chấm công không khớp nhau, còn tệ hơn lỗi ban đầu."),
      },
    ],
    quiz: {
      ...
      options: [
        ...
        },
        {
          ...
          correct: true,
          explanation: B("Yes. Each of those rows is a person. Left alone, they are missing from the run, or paid on the wrong figure. Every one of them is cheaper to fix now than as a retro line next month.",
                         "Đúng vậy. Mỗi dòng đó là một con người. Bỏ mặc thì họ vắng mặt trong đợt lương, hoặc bị trả theo con số sai. Xử lý ngay bây giờ luôn rẻ hơn một dòng hồi tố vào tháng sau."),
        },
        {
  ...
  L5: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("Ten components in four kinds: <b>inputs</b> (base salary), <b>earnings</b> (allowances, overtime), <b>deductions</b> (BHXH, BHYT, BHTN, thuế TNCN) and <b>totals</b> (gross, taxable, net). Below them sit the <b>parameters</b> those formulas read. Nothing on a payslip comes from anywhere else. If a line exists, it is one of these. If a line is wrong, the number behind it is one of these too.",
                "Mười thành phần thuộc bốn loại: <b>đầu vào</b> (lương cơ bản), <b>thu nhập</b> (phụ cấp, tăng ca), <b>khấu trừ</b> (BHXH, BHYT, BHTN, thuế TNCN) và <b>các tổng</b> (tổng thu nhập, thu nhập chịu thuế, thực nhận). Bên dưới là các <b>tham số</b> mà những công thức đó đọc vào. Không dòng nào trên phiếu lương đến từ nơi khác. Có một dòng thì nó là một trong số này. Dòng nào sai thì con số đứng sau nó cũng nằm trong số này."),
      ...
      options: [
        {
          ...
          correct: false,
          explanation: B("Let's rethink that. Computed is not finished. A rejection at any gate cancels the run. Once {{gmTierName}} reopens it, the recompute uses the configuration as it stands THEN. July would quietly gain an allowance nobody approved for it.",
                         "Hãy nghĩ lại một chút. Đã tính không có nghĩa là đã xong. Bị từ chối ở bất kỳ cổng nào cũng huỷ đợt lương. Khi cấp {{gmTierName}} mở lại, lần tính lại sẽ dùng cấu hình TẠI THỜI ĐIỂM ĐÓ. Tháng 7 sẽ âm thầm có thêm một khoản phụ cấp chưa ai duyệt cho nó."),
        },
        {
          ...
          correct: true,
          explanation: B("Yes. The simulation tells you what the change does to real people before anybody is paid by it. Waiting for July to reach done means no open run can pick it up by accident. Both halves matter: the simulation is the evidence, the timing is the safety.",
                         "Đúng vậy. Mô phỏng cho bạn biết thay đổi này tác động thế nào tới người thật trước khi có ai được trả theo nó. Chờ tháng 7 đạt Hoàn tất thì không đợt nào đang mở có thể vô tình nhận phải nó. Cả hai vế đều quan trọng: mô phỏng là bằng chứng, thời điểm là sự an toàn."),
        },
        {
  ...
  LW: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("Payobook does not hide its arithmetic behind fixed salary structures. Every line on every payslip comes from a named component in a <b>formula configuration</b>, one per division. It is written the way a spreadsheet is written, so the person who has to explain it can read it. This card is the way in. The Formula Engine lesson is where you take one apart.",
                "Payobook không giấu phép tính của mình sau các cấu trúc lương cố định. Mọi dòng trên mọi phiếu lương đều đến từ một thành phần có tên trong <b>cấu hình công thức</b>, mỗi bộ phận một bộ. Nó được viết như một bảng tính, nên người phải đi giải thích vẫn đọc được. Thẻ này là lối vào. Còn bài Công thức lương mới là nơi bạn mổ xẻ một bộ."),
      ...
      },
      {
        ...
        body: B("The buttons and cards open screens you could have opened anyway. <b>Run Payroll</b> at the top is the same wizard as the Run Payroll menu item: same rules, same gates, same consequences. That matters. A dashboard with its own shortcut for computing a run would be a second place a run could come from. Nobody could then be sure which one made the month. The two <b>Analytics</b> buttons are the exception. They open an older reporting screen that has no menu item at all, so it is the one door here you cannot find your way back to.",
                "Các nút và thẻ ở đây mở những màn hình mà đằng nào bạn cũng mở được. <b>Chạy bảng lương</b> ở phần đầu chính là trình hướng dẫn của mục Chạy bảng lương: cùng quy tắc, cùng cổng kiểm soát, cùng hậu quả. Điều đó quan trọng. Một bảng điều khiển có lối tắt riêng để tính lương sẽ tạo ra nơi thứ hai sinh ra đợt lương. Khi đó không ai biết chắc tháng này được tạo từ nơi nào. Hai nút <b>Analytics</b> là ngoại lệ. Chúng mở một màn hình báo cáo thế hệ cũ không hề có mục nào trên menu, nên đó là cánh cửa duy nhất ở đây mà bạn không tìm lại được."),
      },
      {
        ...
        body: B("Each row here is one month that was imported, computed, reviewed, approved by every gate and paid. The same loop runs every month, in the same order. A run carries a <b>state</b> rather than a tick because the loop can stop at any point, and somebody has to know where.",
                "Mỗi dòng ở đây là một tháng đã được nhập dữ liệu, tính, soát xét, được mọi cổng phê duyệt và chi trả. Vòng lặp đó chạy lại mỗi tháng, theo đúng thứ tự đó. Một đợt lương mang <b>trạng thái</b> thay vì một dấu tích, vì vòng lặp có thể dừng ở bất kỳ điểm nào và phải có người biết nó dừng ở đâu."),
        moment: { kind: "pipeline", chain: "payrun" },
      },
      {
        ...
        body: B("<b>Overview</b> is where you land, and where approvals queue. <b>Pay Run</b> is the month's work, top to bottom. <b>People</b> holds who can be paid. <b>Insights</b> answers questions about what was paid. <b>Compliance</b> is the filings the authorities ask for, and <b>Setup</b> is read often and changed rarely. An item missing from your menu is one your groups do not open. The Journey still describes it, because somebody who cannot open a screen is exactly the person who needs to know what it is.",
                "<b>Tổng quan</b> là nơi bạn vào đầu tiên, và là nơi các lượt phê duyệt xếp hàng. <b>Chạy lương</b> là toàn bộ công việc của tháng, từ đầu đến cuối. <b>Nhân sự</b> giữ thông tin ai có thể được trả lương. <b>Phân tích</b> trả lời các câu hỏi về những gì đã chi. <b>Tuân thủ</b> là các báo cáo mà cơ quan quản lý yêu cầu, còn <b>Thiết lập</b> thì hay được đọc và hiếm khi bị sửa. Mục nào thiếu trong menu của bạn là mục nhóm quyền của bạn không mở được. Hành trình học vẫn mô tả nó, vì người không mở được một màn hình chính là người cần biết màn hình đó là gì."),
        tip: B("The menu on the left of this practice screen has the shape of the real one, not its exact contents. Your own menu also carries a <b>Learning</b> section, and that is where the Journey you are reading lives.",
               "Menu bên trái màn hình thực hành này mô phỏng hình dạng của menu thật, không phải nội dung chính xác của nó. Menu của chính bạn còn có nhóm mục <b>Học tập</b>, và đó là nơi chứa Hành trình học bạn đang đọc."),
      },
      {
        ...
        body: B("The launcher in the bottom-right corner opens on <b>any</b> screen in Payobook. It answers about the screen you are standing on. What it is, what to do next here, what a tile counts, and what a control would do before you press it. It explains — you act. It never computes a run, approves a payslip or changes a record. It never invents a rate either: every answer it gives is something a person wrote and a test checks.",
                "Nút trợ giúp ở góc dưới bên phải mở được trên <b>mọi</b> màn hình của Payobook. Nó trả lời về đúng màn hình bạn đang đứng. Đây là gì, ở đây nên làm gì tiếp, một ô số liệu đang đếm cái gì, và một nút sẽ gây ra điều gì trước khi bạn bấm. Nó giải thích — bạn thao tác. Nó không bao giờ tự tính lương, phê duyệt phiếu lương hay sửa dữ liệu. Nó cũng không bao giờ bịa ra một tỷ lệ: mọi câu trả lời đều do con người viết và có kiểm thử canh giữ."),
    ...
    quiz: {
      ...
      options: [
        ...
        {
          text: B("Forty-eight payslips are waiting somewhere in the HR and Finance tiers, and Approvals will tell me how many are mine", "Có bốn mươi tám phiếu lương đang chờ đâu đó trong cấp HR và Tài chính, và màn hình Phê duyệt sẽ cho biết bao nhiêu là của tôi"),
          correct: true,
          explanation: B("Exactly. The Dashboard reports a company-wide state, counted in payslips. The queue that is YOURS is counted in runs, on its own screen, at the gate your groups let you act on. Read the state first and the queue second — that is the order the whole product is built in.",
                         "Chính xác. Bảng điều khiển báo cáo hiện trạng toàn công ty, đếm theo số phiếu lương. Hàng đợi CỦA BẠN được đếm theo đợt, trên màn hình riêng của nó, tại đúng cổng mà nhóm quyền của bạn cho phép xử lý. Hãy đọc hiện trạng trước rồi mới tới hàng đợi — đó là đúng trật tự mà cả sản phẩm này được dựng lên."),
        },
        {
          text: B("Payroll is behind schedule", "Công việc tính lương đang chậm tiến độ"),
          correct: false,
          explanation: B("Let's rethink that. Payslips sitting at a gate mid-month is what a working approval chain looks like. The number only means \"behind\" once you know which gate they are at and how long they have been there. This tile carries neither of those.",
                         "Hãy nghĩ lại một chút. Giữa tháng có phiếu lương nằm chờ ở các cổng chính là hình ảnh của một chuỗi phê duyệt đang hoạt động. Con số này chỉ có nghĩa là \"chậm\" khi bạn biết chúng đang ở cổng nào và đã nằm đó bao lâu. Ô này không mang theo điều nào trong hai điều đó."),
        },
      ],
    },
  },
  ...
  LA: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: B("Three counts, one per gate, and then <b>612,480,000 ₫</b> — the net of every run in the pipeline. Read it as what it is. This is money one or more signatures away from leaving the company's account, and anybody who finds a reason can still stop it.",
                "Ba con số đếm, mỗi cổng một con số, rồi tới <b>612.480.000 ₫</b> — tổng thực nhận của mọi đợt đang trong quy trình. Hãy đọc nó đúng bản chất. Đây là số tiền chỉ còn cách tài khoản công ty một hoặc vài chữ ký, và bất kỳ ai tìm ra lý do đều còn kịp chặn lại."),
      ...
      },
      {
        ...
        body: B("July is 612,480,000 ₫ against June's 596,110,000 ₫. Those are run totals, which live on the board and the KPI band rather than on the payslip in front of you. This step stands here anyway, because the answer does. Toggle the two and say out loud why they differ: one more employee, and one payslip carrying 3,100,000 ₫ more overtime than last month. If you cannot finish that sentence, the gap is not small. It is unexamined.",
                "Tháng 7 là 612.480.000 ₫ so với 596.110.000 ₫ của tháng 6. Đó là tổng của cả đợt, nằm trên bảng và dải chỉ số chứ không nằm trên phiếu lương trước mặt bạn. Bước này vẫn đứng ở đây, vì câu trả lời nằm ở đây. Hãy chuyển qua lại hai bên và nói thành lời vì sao chúng khác nhau: thêm một nhân viên, và một phiếu lương mang tăng ca nhiều hơn tháng trước 3.100.000 ₫. Nếu bạn chưa nói trọn được câu đó thì khoảng chênh này không nhỏ. Nó chỉ là chưa được soi tới."),
      ...
      },
      {
        ...
        body: B("Nothing about the button says so, which is why this step does. Pressing Approve moves 48 payslips one gate closer to being paid, all together. There is no way to approve only some of them. The gate after yours checks totals rather than lines. If a wrong line is going to be caught at all, this is one of the last places it can be.",
                "Cái nút không hề nói ra điều đó, nên bước này phải nói. Bấm Phê duyệt là đẩy 48 phiếu lương tiến thêm một cổng tới lúc được chi, tất cả cùng lúc. Không có cách nào chỉ duyệt một phần. Cổng sau bạn kiểm tra các con số tổng chứ không kiểm tra từng dòng. Nếu một dòng sai còn có thể bị bắt, thì đây là một trong những nơi cuối cùng bắt được."),
        consequence: B("Affects the whole run, all 48 payslips together. It moves from your gate to the next one. Reversible: <b>stoppable, not undoable</b>. A later reviewer can still reject it, which cancels the batch rather than handing it back. After done, even that is gone, and a correction becomes a retro line. Verify first: every flagged payslip opened, a few unflagged ones sampled, and a one-sentence explanation of the variance against last month.",
                       "Ảnh hưởng cả đợt lương, toàn bộ 48 phiếu cùng lúc. Đợt chuyển từ cổng của bạn sang cổng kế tiếp. Hoàn tác: <b>chặn được, chứ không lùi lại được</b>. Người soát xét sau vẫn có thể từ chối, và điều đó huỷ cả lô chứ không trả nó lại cho bạn. Sau khi Hoàn tất thì đến cách đó cũng không còn, và mọi hiệu chỉnh trở thành một dòng hồi tố. Kiểm tra trước: đã mở mọi phiếu bị gắn cờ, đã lấy mẫu vài phiếu không gắn cờ, và giải thích được biến động so với tháng trước trong một câu."),
      },
      {
        ...
        body: B("Approvals and rejections land in the same list, and a rejection keeps its written reason where the next person can read it. That is not surveillance. It is how a payroll month can still be defended six months later, when nobody remembers the conversation and the record is all there is.",
                "Phê duyệt và từ chối cùng rơi vào một danh sách, và một lần từ chối vẫn giữ nguyên lý do bằng văn bản ở nơi người sau đọc được. Đó không phải là giám sát. Đó là cách một kỳ lương vẫn bảo vệ được sau sáu tháng, khi không ai còn nhớ cuộc trao đổi nào và chỉ còn lại hồ sơ."),
      },
    ],
    quiz: {
      ...
      options: [
        ...
        },
        {
          ...
          explanation: B("Let's rethink that. Rejecting CANCELS all 48 payslips and puts a reason on the permanent record — and \"please check everything\" is not a reason anybody can act on. Worse, the officer cannot simply pick the run back up: reopening it as a draft is a {{gmTierName}} action. A rejection is testimony about a problem you found, not a way of asking somebody to go looking for one.",
                         "Hãy nghĩ lại một chút. Từ chối sẽ HUỶ cả 48 phiếu lương và ghi một lý do vào hồ sơ vĩnh viễn — mà \"vui lòng kiểm tra lại toàn bộ\" thì không phải lý do ai xử lý được. Tệ hơn, chuyên viên không thể tự nhặt đợt lương đó lên làm tiếp: mở lại nó thành bản nháp là thao tác của cấp {{gmTierName}}. Từ chối là lời chứng về một vấn đề bạn đã phát hiện, không phải cách nhờ người khác đi tìm xem có vấn đề gì không."),
        },
      ],
    },
  },
...
const MISSIONS = [
  {
    ...
    anomaly: {
      ...
      body: B("Trần Văn Hùng's overtime came through at 4,200,000 ₫ against 1,100,000 ₫ in June — 382%. It reads like a genuine peak week. That is exactly why it is worth stopping on. A decimal point dropped in a timesheet produces a number that is believable, not absurd. The engine cannot tell a busy month from a typo. You can, by opening the timesheet. The cost of not looking is one employee paid roughly three million đồng too much, in a run that forty-seven other people will assume was checked.",
             "Tăng ca của Trần Văn Hùng vào hệ thống ở mức 4.200.000 ₫ so với 1.100.000 ₫ của tháng 6 — bằng 382%. Nó trông như một tuần cao điểm thật. Chính vì thế mới đáng dừng lại. Một dấu thập phân bị mất trong bảng chấm công tạo ra con số dễ tin, chứ không hề vô lý. Hệ thống không phân biệt được tháng bận với lỗi gõ. Bạn thì phân biệt được, bằng cách mở bảng chấm công. Cái giá của việc không xem là một nhân viên được trả thừa khoảng ba triệu đồng, trong một đợt lương mà bốn mươi bảy người khác mặc định là đã được kiểm tra."),
    },
    debrief: {
      did: [
  ...
  },
  {
    ...
    consequence: {
      ...
      reversible: B("Stoppable at this gate rather than undoable. The next reviewer can still reject it, which cancels the whole batch. Reopening a cancelled run as a draft is a {{gmTierName}} action. At done even that is gone, and a correction becomes a retro line.",
                    "Ở cổng này thì chặn được chứ không lùi lại được. Người soát xét kế tiếp vẫn có thể từ chối, và điều đó huỷ cả lô. Mở lại một đợt đã bị từ chối thành bản nháp là thao tác của cấp {{gmTierName}}. Khi đã Hoàn tất thì đến cách đó cũng không còn, và mọi hiệu chỉnh trở thành một dòng hồi tố."),
    ...
    anomaly: {
      ...
      body: B("The run's net total is 612,480,000 ₫ against 596,110,000 ₫ in June. That is a rise of about 2.7%, on one more employee. It is a comfortable-looking number, and comfortable-looking numbers are where one wrong payslip hides best. Hùng's overtime alone accounts for roughly three million of the difference. That sits well inside the range a reviewer waves through as a slightly busier month. A total that looks reasonable is not evidence that every payslip inside it is. The flags are.",
             "Tổng thực nhận của đợt là 612.480.000 ₫ so với 596.110.000 ₫ tháng 6. Tức tăng khoảng 2,7%, với thêm một nhân viên. Đó là con số trông rất dễ chịu, và những con số dễ chịu chính là nơi một phiếu lương sai ẩn mình tốt nhất. Riêng tăng ca của Hùng đã chiếm khoảng ba triệu trong phần chênh đó. Mức đó nằm gọn trong khoảng mà một người soát xét sẽ cho qua như một tháng bận hơn chút. Tổng trông hợp lý không phải bằng chứng rằng từng phiếu bên trong đều hợp lý. Các cờ cảnh báo mới là bằng chứng."),
    },
    debrief: {
      did: [
        ...
        B("Watched a rejection cancel the batch, and found out who has to reopen it. The cost of that decision is now something you have seen rather than been told.",
          "Xem một lần từ chối huỷ cả lô, và biết được ai mới là người mở lại nó. Cái giá của quyết định ấy giờ là điều bạn đã tận mắt thấy, chứ không phải chỉ nghe kể."),
      ],
      checklist: [
     ...
     ======================================================================== */
  {
    ...
    desc: B("The same judgement as the practice run, but on real records in the demo world. Your own division, the open June period, and the approval chain all the way to done.",
    ...
    consequence: {
      ...
      reversible: B("Partly, and the halves matter. Draft payslips can be deleted and recomputed as often as you like. Submitting is not silently undoable. From there the run moves only by being approved, or by being rejected. A rejection cancels the batch, and the {{gmTierName}} tier has to reopen it as a draft. The same chain a real payroll travels.",
                    "Một phần, và ranh giới rất quan trọng. Phiếu lương nháp có thể xoá và tính lại bao nhiêu lần tuỳ ý. Trình phê duyệt thì không thể âm thầm hoàn tác. Từ đó đợt lương chỉ đi tiếp bằng cách được duyệt, hoặc bị từ chối. Từ chối là huỷ cả lô, và cấp {{gmTierName}} phải mở lại nó thành bản nháp. Đúng chuỗi mà một kỳ lương thật đi qua."),
      verify: B("That the division the wizard has selected is the one assigned to you. It is preselected and moved to the front of the list for exactly this reason. The mission checks against your assignment, not against whatever the wizard happens to be showing.",
                "Rằng bộ phận mà trình hướng dẫn đang chọn đúng là bộ phận được gán cho bạn. Nó được chọn sẵn và đưa lên đầu danh sách chính vì lý do này. Nhiệm vụ kiểm tra theo phần được gán cho bạn, chứ không theo thứ mà trình hướng dẫn đang hiển thị."),
    },
    debrief: {
      did: [
...
const MISSION_STEPS = {
  m1: [
    ...
    },
    {
      ...
      recovery: {
        fnb: B("Let's rethink that. F&B's July attendance has not been committed yet. The eligible count would come back far short of its headcount. Computing on it would create a run that looks finished and is missing most of its people.",
               "Hãy nghĩ lại một chút. Chấm công tháng 7 của F&B chưa được ghi nhận. Số đủ điều kiện sẽ thiếu rất nhiều so với sĩ số. Tính trên đó sẽ tạo ra một đợt lương trông như đã xong nhưng thiếu phần lớn nhân sự."),
    ...
    },
    {
      ...
      recovery: {
        accept: B("Let's rethink that. 4.6 hours typed as 46 is one of the most common timesheet errors there is. It produces a number that looks like a busy month rather than a mistake. Accepting an unverified spike is not a decision. It is a decision handed to whoever notices on {{payDay}}.",
                  "Hãy nghĩ lại một chút. 4,6 giờ bị gõ thành 46 là một trong những lỗi chấm công phổ biến nhất. Nó tạo ra con số trông giống tháng bận chứ không giống lỗi. Chấp nhận một mức tăng chưa xác minh không phải là ra quyết định. Đó là đẩy quyết định cho người phát hiện ra vào {{payDay}}."),
        zero: B("Let's rethink that. That fixes the result and leaves the input untouched, so the next recompute brings it straight back. And if the overtime was real, you have just underpaid somebody who worked for it. Fix inputs, never results.",
                "Hãy nghĩ lại một chút. Cách đó sửa kết quả và để nguyên đầu vào, nên lần tính lại kế tiếp mang nó quay lại ngay. Và nếu tăng ca là thật, bạn vừa trả thiếu cho người đã làm. Hãy sửa đầu vào, đừng bao giờ sửa kết quả."),
      },
    },
    {
    ...
    },
    {
      ...
      detail: B("A submitted run can still be STOPPED, and that is not the same as undone. Rejecting cancels the whole batch, all 48 payslips with it, and records your reason. Getting a workable draft back afterwards is a {{gmTierName}} action, not yours. Do it once here. Then on a real run you already know both halves. There is a way to stop money moving, and it costs the people who have to restart it.",
                "Một đợt đã trình vẫn CHẶN được, và điều đó không giống với hoàn tác. Từ chối sẽ huỷ cả lô, kèm toàn bộ 48 phiếu lương, và ghi lại lý do của bạn. Để có lại một bản nháp làm việc được thì đó là thao tác của cấp {{gmTierName}}, không phải của bạn. Hãy làm một lần ở đây. Khi gặp đợt lương thật bạn đã biết cả hai vế. Có cách chặn dòng tiền lại, và nó có cái giá với những người phải khởi động lại."),
  ...
  m2: [
    {
      ...
      detail: B("Time has moved on since m1: the Payroll Officer has approved the July run at their tier, so it now sits at {{hrTierName}} — yours. Awaiting your approval is the only number on this band that is about you.",
                "Thời gian đã trôi qua kể từ m1: Chuyên viên tính lương đã duyệt đợt tháng 7 ở cấp của họ, nên nó đang nằm ở {{hrTierName}} — cấp của bạn. Chờ bạn phê duyệt là con số duy nhất trên dải này nói về bạn."),
    ...
    },
    {
      ...
      recovery: {
        ...
        wait: B("Let's rethink that. Once you approve, the next gate checks totals rather than lines. Your email is now racing a run that has already moved on. If it lands late, the correction is a retro line rather than a fix.",
                "Hãy nghĩ lại một chút. Khi bạn đã duyệt, cổng kế tiếp kiểm tra các tổng chứ không kiểm tra từng dòng. Email của bạn đang chạy đua với một đợt lương đã đi tiếp. Nếu nó tới muộn thì hiệu chỉnh là một dòng hồi tố, chứ không còn là sửa lỗi."),
      },
    },
    {
    ...
    },
    {
      ...
      detail: B("The run now reads <b>Rejected</b> and all 48 payslips are cancelled with it, your reason on the record. It does NOT come back by itself. Reopening it as a draft is a {{gmTierName}} action. Only then can the officer correct the input, recompute and resubmit through the same chain. Nothing was paid. But a rejection costs a second person's time as well as the officer's, which is why the reason has to be worth acting on.",
                "Đợt lương giờ ở trạng thái <b>Đã từ chối</b> và cả 48 phiếu lương bị huỷ theo, kèm lý do của bạn trong hồ sơ. Nó KHÔNG tự quay lại. Mở lại thành bản nháp là thao tác của cấp {{gmTierName}}. Chỉ sau đó chuyên viên mới sửa được dữ liệu đầu vào, tính lại và trình lại qua đúng chuỗi đó. Chưa có gì được chi. Nhưng một lần từ chối tốn thời gian của cả người thứ hai chứ không chỉ của chuyên viên, và đó là lý do bản thân lý do phải đáng để người ta hành động."),
    },
  ],
  ...
  mL1: [
    ...
    },
    {
      ...
      instruction: B("Open Run Payroll", "Mở Chạy bảng lương"),
      detail: B("Your division is already selected and moved to the top of the list. The period is pinned to June 2026, the one month the demo world leaves open. Read both before you go on.",
                "Bộ phận của bạn đã được chọn sẵn và đưa lên đầu danh sách. Kỳ lương được cố định ở tháng 6/2026, tháng duy nhất mà môi trường demo để mở. Hãy đọc cả hai trước khi đi tiếp."),
    ...
    },
    {
      ...
      instruction: B("Submit the run for approval", "Trình đợt lương lên phê duyệt"),
      detail: B("This is the step that stops being reversible in the easy way. After it, the run moves only by being approved at each gate, or by being rejected. A rejection cancels the batch rather than handing it back to you.",
                "Đây là bước mà việc hoàn tác không còn dễ dàng nữa. Sau bước này, đợt lương chỉ đi tiếp bằng cách được duyệt ở từng cổng, hoặc bị từ chối. Từ chối là huỷ cả lô chứ không trả nó lại cho bạn."),
    ...
    },
    {
      ...
      detail: B("On the demo world your account holds the approval tiers, so you can walk the run through the chain yourself. On a real tenant these are four different people, and the waiting between them is most of what payroll week is.",
                "Trên môi trường demo, tài khoản của bạn giữ các cấp phê duyệt, nên bạn tự đưa đợt lương đi hết chuỗi được. Ở một hệ thống thật, đây là bốn con người khác nhau, và phần lớn một tuần tính lương chính là những khoảng chờ giữa họ."),
      hint: B("The column a run sits in names the tier that has to act next. That is the answer to \"who do I chase\".",
              "Cột mà đợt lương đang nằm chỉ đích danh cấp phải xử lý tiếp theo. Đó chính là câu trả lời cho \"tôi phải hỏi ai\"."),
    },
    {
...
const SCREEN_CTX = {
  runpayroll: {
    blurb: B("The wizard that builds one month of draft payslips for one division.",
             "Trình hướng dẫn dựng một tháng phiếu lương nháp cho một bộ phận."),
  ...
  payruns: {
    blurb: B("Every pay run on one board. Each sits in the column of the gate it is waiting at.",
             "Mọi đợt tính lương trên một bảng. Mỗi đợt nằm ở cột của cổng nó đang chờ."),
  ...
  import: {
    blurb: B("Where attendance, overtime and HR data arrive. It comes from files and connected systems, before any of it becomes a payslip.",
             "Nơi dữ liệu chấm công, tăng ca và nhân sự đi vào. Nó đến từ tệp và các hệ thống đã kết nối, trước khi bất kỳ phần nào trở thành phiếu lương."),
    next: B("Start an import to use the guided flow. It scores the file and lets you fix rows before anything is written. A batch that stops before commit has changed nothing.",
            "Hãy bắt đầu một đợt nhập để dùng luồng có hướng dẫn. Nó chấm điểm tệp và cho bạn sửa các dòng trước khi ghi bất cứ gì. Một đợt dừng trước bước ghi nhận thì chưa thay đổi gì cả."),
  ...
  importwizard: {
    ...
    next: B("Resolve every row in \"Need attention\" before you commit. Commit is the first control here that writes anything. Everything before it is a preview.",
            "Hãy xử lý mọi dòng trong \"Cần xử lý\" trước khi ghi nhận. Ghi nhận là nút đầu tiên ở đây thực sự ghi dữ liệu. Mọi thứ trước đó chỉ là xem trước."),
  ...
  proration: {
    ...
    next: B("Read the factor, not the money. If the factor is right and the amount still looks wrong, the problem is further back: the contract, or the import.",
            "Hãy đọc hệ số chứ không phải số tiền. Nếu hệ số đúng mà số tiền vẫn có vẻ sai thì vấn đề nằm ở phía trước đó: hợp đồng, hoặc đợt nhập liệu."),
    chips: ["prorata", "whydiff", "whatpage"],
  },
  retro: {
    blurb: B("Money owed for a month that is already closed. It is paid in the current month, and every line names the month it came from.",
             "Khoản tiền còn nợ của một tháng đã đóng. Nó được chi trong tháng hiện tại, và mỗi dòng đều nêu rõ tháng gốc."),
  ...
  /* -- Setup ------------------------------------------------------------- */
  formula: {
    blurb: B("The rulebook, out in the open. Every payslip line for one division is a named component with a formula you can read.",
             "Bộ quy tắc bày ra rõ ràng. Mỗi dòng phiếu lương của một bộ phận là một thành phần có tên, kèm công thức bạn đọc được."),
  ...
  structures: {
    ...
    next: B("Read, do not build. New pay logic belongs in a formula configuration. What is here exists so that payslips from before the move can still be explained.",
  ...
  integrations: {
    blurb: B("The connectors payroll data arrives through: an HR system, a time clock, the bank. Each shows its field mapping and its sync history.",
             "Các đầu nối mà dữ liệu tính lương đi vào qua đó: hệ thống nhân sự, máy chấm công, ngân hàng. Mỗi đầu nối hiện ánh xạ trường và lịch sử đồng bộ của nó."),
    next: B("Read the last-sync time on every connector, not just its status. Connected describes the login. The sync time describes the data. A connector that stopped looks exactly like one that is working.",
            "Hãy đọc thời điểm đồng bộ gần nhất của từng đầu nối, không chỉ đọc trạng thái. Đã kết nối nói về thông tin đăng nhập. Thời điểm đồng bộ mới nói về dữ liệu. Một đầu nối đã ngừng chạy trông y hệt một đầu nối đang chạy tốt."),
    chips: ["syncbroken", "whysetup", "whatnext"],
  },

  /* -- Overview, People, Insights, Compliance (Phase C1) ------------------ */
  dashboard: {
    blurb: B("Your starting screen. The live run, four numbers that describe the company, and a card for each part of Payobook worth opening today.",
  ...
  approvals: {
    blurb: B("Every submitted run, in the lane of the gate it is waiting at. The header adds up the money at stake across all of them.",
             "Mọi đợt đã trình phê duyệt, nằm ở làn của cổng nó đang chờ. Phần đầu cộng lại số tiền đang treo trên tất cả."),
    next: B("Read the count in the header. That is the part of this queue only you can move. Then open the run rather than deciding from its card. The total is on the card; the flags are on the payslips.",
            "Hãy đọc con số ở tiêu đề. Đó là phần hàng đợi chỉ bạn mới đẩy đi được. Rồi mở đợt lương ra thay vì quyết định ngay trên thẻ. Con số tổng nằm trên thẻ; cờ cảnh báo nằm trên từng phiếu lương."),
  ...
  insights: {
    blurb: B("A board of analytics built from the stored per-run roll-ups. The cost story, the department leaderboard, the statutory split and the snapshots.",
             "Bảng phân tích dựng từ các số tổng đã lưu theo từng đợt. Diễn biến chi phí, xếp hạng bộ phận, cơ cấu đóng bắt buộc và các ảnh chụp số liệu."),
...
const QA = [
...

  {
    ...
    roleVariants: {
      any: [
        { k: "p", v: B("A run is approved one gate at a time: draft, then the Payroll Officer tier, then {{hrTierName}}, then {{gmTierName}}, then done. Each gate belongs to one group. The buttons on a card come from the record's own gate fields and your groups, so what you can see is what you can do.",
                       "Một đợt lương được duyệt lần lượt qua từng cổng: Nháp, rồi cấp Chuyên viên tính lương, rồi {{hrTierName}}, rồi {{gmTierName}}, rồi Hoàn tất. Mỗi cổng thuộc về một nhóm quyền. Các nút trên thẻ đến từ chính các trường kiểm soát cổng của bản ghi và nhóm quyền của bạn, nên thấy được gì là làm được nấy.") },
      ...
      manager: [
        { k: "ok", v: B("Yes. You hold an approval gate, so a run waiting at your tier will show Approve on its card. Approving moves all 48 payslips together — there is no way to approve some of them.",
                        "Có. Bạn giữ một cổng phê duyệt, nên đợt lương đang chờ ở cấp của bạn sẽ hiện nút Phê duyệt trên thẻ. Phê duyệt sẽ chuyển toàn bộ 48 phiếu cùng lúc — không có cách nào duyệt một phần.") },
        { k: "steps", v: [
          { t: B("Open the run in the column that names your tier", "Mở đợt lương ở cột mang tên cấp của bạn"), a: "pk-tabs" },
      ...
      owner: [
        { k: "ok", v: B("Yes — you hold every gate. Use that carefully rather than often. A chain where one person signs every tier records one opinion four times, and the point of the tiers is that they are four different reads.",
                        "Có — bạn giữ mọi cổng. Hãy dùng điều đó một cách thận trọng, đừng dùng thường xuyên. Một chuỗi mà một người ký mọi cấp chỉ ghi lại một ý kiến bốn lần, trong khi ý nghĩa của các cấp là bốn lần soát khác nhau.") },
      ...
      operator: [
        { k: "p", v: B("You own the first gate: you can submit a draft run for review and approve at the Payroll Officer tier. {{hrTierName}} and {{gmTierName}} come after you and belong to other people.",
                       "Bạn giữ cổng đầu tiên: bạn có thể trình một đợt nháp lên soát xét và duyệt ở cấp Chuyên viên tính lương. {{hrTierName}} và {{gmTierName}} nằm sau bạn và thuộc về người khác.") },
        { k: "steps", v: [
          ...
          { t: B("Approve your own tier once your checks pass", "Duyệt cấp của bạn khi các bước kiểm tra đã đạt"), a: "pk-card-actions" },
        ] },
        { k: "src", v: B("The pay run's approval chain and your group membership.",
                         "Chuỗi phê duyệt của đợt lương và nhóm quyền của bạn.") },
      ],
      no_access: [
        { k: "refusal", v: B("Not from here. Approving a pay run needs one of the payroll approval groups. This screen is not in your menu, so there is no button for you to be missing.",
                             "Không phải từ đây. Phê duyệt một đợt lương cần một trong các nhóm quyền phê duyệt lương. Màn hình này không có trong menu của bạn, nên không có nút nào bạn đang thiếu cả.") },
        { k: "who", v: B("The Payroll Officer tier, {{hrTierName}} and {{gmTierName}} each belong to a different group. The column a run is sitting in names the tier that has to act next.",
                         "Cấp Chuyên viên tính lương, {{hrTierName}} và {{gmTierName}} mỗi cấp thuộc một nhóm quyền khác nhau. Cột mà đợt lương đang nằm chỉ đích danh cấp phải xử lý tiếp theo.") },
        { k: "how", v: B("Ask {{payrollSupportContact}}. Access to an approval tier is a decision about who signs for money. It is granted deliberately rather than on request, so expect to be asked what you need it for.",
                         "Hãy hỏi {{payrollSupportContact}}. Quyền vào một cấp phê duyệt là quyết định về việc ai ký cho những khoản tiền. Nó được cấp có cân nhắc chứ không phải cứ xin là có, nên hãy chuẩn bị trả lời bạn cần nó để làm gì.") },
...

  {
    ...
    showMe: ["pk-card-actions"],
    blocks: [
      { k: "p", v: B("The whole run is <b>cancelled</b>: all 48 payslips together, never one. Its state becomes Rejected. Three things are recorded against it: who rejected it, when, and why in writing.",
                     "Cả đợt lương bị <b>huỷ</b>: toàn bộ 48 phiếu cùng lúc, không bao giờ chỉ một phiếu. Trạng thái chuyển thành Đã từ chối. Ba điều được ghi lại: ai từ chối, vào lúc nào, và vì sao, bằng văn bản.") },
      { k: "steps", v: [
        ...
        { t: B("Somebody at the {{gmTierName}} tier reopens the cancelled run as a draft — this is the part people expect to be automatic and is not", "Một người ở cấp {{gmTierName}} mở lại đợt đã huỷ thành bản nháp — đây là khâu người ta hay tưởng là tự động, nhưng không phải") },
      ...
      { k: "warn", v: B("Nothing was paid, but this is blunter than it looks. The batch is cancelled, not handed back. It then takes a second person at the {{gmTierName}} tier before the officer can even start. A reason like \"wrong\" costs all of that, plus a day of guessing.",
                        "Chưa có gì được chi, nhưng thao tác này thô hơn vẻ ngoài của nó. Cả lô bị huỷ chứ không phải được trả lại. Rồi phải có người thứ hai ở cấp {{gmTierName}} thì chuyên viên mới bắt đầu lại được. Một lý do kiểu \"sai\" khiến người ta trả toàn bộ cái giá đó, cộng thêm cả ngày đoán mò.") },
...

  {
    ...
    blocks: [
      { k: "p", v: B("It is the share of rows the importer matched to an employee and read without ambiguity — 46 of 48 here. It is not a grade and there is no pass mark. The number that matters is the count underneath it: 95.8% of 48 rows still leaves two rows a person has to resolve.",
                     "Đó là tỷ lệ dòng mà trình nhập liệu khớp được với một nhân viên và đọc được rõ ràng, không nhập nhằng — ở đây là 46 trên 48. Nó không phải điểm số và không có mức đạt. Con số quan trọng là số lượng bên dưới nó: 95,8% của 48 dòng vẫn còn hai dòng phải có người xử lý.") },
...

  {
    ...
    practice: "m1",
    blocks: [
      { k: "p", v: B("Good instinct. The practice missions run on a made-up 48-person company, not {{companyDisplayName}}. There is no server behind it, so nothing you do can reach a real employee, payslip or pay run. Three are playable: computing a run, reviewing one at an approval gate, and applying a statutory rate change.",
                     "Bản năng tốt. Các nhiệm vụ thực hành chạy trên một công ty giả lập 48 người, không phải {{companyDisplayName}}. Phía sau nó không có máy chủ nào, nên mọi thao tác đều không chạm tới nhân viên, phiếu lương hay đợt lương thật. Ba nhiệm vụ chơi được: tính một đợt lương, soát xét một đợt ở cổng phê duyệt, và áp dụng một thay đổi tỷ lệ luật định.") },
      { k: "ok", v: B("You can fail safely there. The seeded anomaly is always present, so the judgement it teaches is the same one every time. Getting it wrong costs a recovery message rather than a salary.",
                      "Bạn được phép sai ở đó. Điểm bất thường được cài sẵn luôn có mặt, nên phán đoán mà nó dạy luôn là một. Làm sai chỉ tốn một lời gợi ý quay lại, chứ không tốn một khoản lương.") },
...

  {
    ...
    blocks: [
      ...
      { k: "p", v: B("You do not have to look it up before running a payroll. Choosing the division in Run Payroll selects the configuration, and prints its name in the scope panel. Reading that name before you compute is the check. It is also the only place the wrong-division mistake is visible.",
                     "Bạn không cần tra cứu trước khi chạy lương. Chọn bộ phận trong Chạy bảng lương là đã chọn cấu hình, và tên của nó được in ở bảng phạm vi. Đọc cái tên đó trước khi tính chính là bước kiểm tra. Đó cũng là nơi duy nhất nhìn thấy được lỗi chọn nhầm bộ phận.") },
...

  {
    ...
    showMe: ["pa-lanes"],
    blocks: [
      { k: "p", v: B("The lane a run is sitting in names the tier that has to act next: Officer review, then {{hrTierName}}, then {{gmTierName}}. That is the answer to \"who do I chase\". It is also more reliable than asking, because the record decides it rather than somebody's memory of who offered to look.",
                     "Làn mà một đợt lương đang nằm chỉ đích danh cấp phải xử lý tiếp theo: Chuyên viên soát, rồi {{hrTierName}}, rồi {{gmTierName}}. Đó chính là câu trả lời cho \"tôi phải hỏi ai\". Nó cũng đáng tin hơn việc đi hỏi, vì chính bản ghi quyết định điều đó chứ không phải trí nhớ của ai về việc đã hứa sẽ xem.") },
      { k: "p", v: B("A card offers Approve and Reject only at the gate you hold. If you cannot see those buttons on a run, it is not yours to move. Pressing harder will not change that: the buttons come from the record's own gate fields and your groups.",
                     "Một thẻ chỉ hiện nút Phê duyệt và Từ chối ở đúng cổng bạn đang giữ. Nếu bạn không thấy hai nút đó trên một đợt lương thì đợt đó không phải phần việc của bạn. Bấm mạnh hơn cũng không đổi được: các nút đến từ chính các trường kiểm soát cổng của bản ghi và nhóm quyền của bạn.") },
...

  {
    ...
    practice: "m2",
    simpler: B("Not all of them. Payobook has already marked the ones it was unsure about. Read those first. Then pick two or three ordinary ones at random, to make sure the ordinary ones are fine too.",
               "Không phải tất cả. Payobook đã đánh dấu sẵn những phiếu mà nó không chắc. Hãy đọc những phiếu đó trước. Rồi chọn ngẫu nhiên hai ba phiếu bình thường, để chắc rằng các phiếu bình thường cũng ổn."),
    blocks: [
...

  {
    ...
    blocks: [
      { k: "p", v: B("\"Normal\" is not a percentage. It is whether you can finish the sentence. On this run, July is 612,480,000 ₫ against June's 596,110,000 ₫. That is a rise of 2.7% on one more employee, and one payslip carries 3,100,000 ₫ more overtime than last month.",
                     "\"Bình thường\" không phải là một con số phần trăm. Nó là việc bạn có nói trọn được câu giải thích hay không. Trên đợt này, tháng 7 là 612.480.000 ₫ so với 596.110.000 ₫ của tháng 6. Tức tăng 2,7% với thêm một nhân viên, và một phiếu lương có tăng ca nhiều hơn tháng trước 3.100.000 ₫.") },
...

  {
    ...
    blocks: [
      ...
      { k: "warn", v: B("\"Wrong\", \"please recheck\" and \"the numbers look off\" all do the same three things. They cancel 48 payslips, they need a {{gmTierName}} reset before anybody can start again, and they leave the officer guessing at what you saw. They will probably rebuild the same thing, and the second rejection is the one that becomes an argument.",
                        "\"Sai\", \"kiểm tra lại giúp\" hay \"số liệu trông không ổn\" đều gây ra ba việc như nhau. Chúng huỷ 48 phiếu lương, chúng cần cấp {{gmTierName}} mở lại thì mới có ai bắt đầu lại được, và chúng để chuyên viên tự đoán xem bạn đã thấy gì. Nhiều khả năng họ sẽ dựng lại đúng thứ cũ, và lần từ chối thứ hai mới là lần biến thành tranh cãi.") },
...
const COLUMNS = {
  ...
  payruns: [
    ...
    ["awaiting_your_approval",
     ...
     B("Runs sitting at a gate that your groups let you act on. This is the only number on the band that is about you. Read it first, every morning of payroll week. Nothing here moves until you move it.",
       "Các đợt đang nằm ở cổng mà nhóm quyền của bạn cho phép xử lý. Đây là con số duy nhất trên dải này nói về bạn. Hãy đọc nó đầu tiên, mỗi sáng trong tuần tính lương. Không gì ở đây nhúc nhích cho tới khi bạn động vào.")],
    ["completed",
     B("Completed", "Hoàn tất"),
     B("Runs that have passed every gate. Completion unlocks the bank file, the journals and the payments. It is also the point after which a correction becomes a retro line rather than an edit.",
       "Các đợt đã qua mọi cổng phê duyệt. Hoàn tất mở khoá tệp chi lương, bút toán và các khoản thanh toán. Đó cũng là mốc mà sau đó mọi hiệu chỉnh trở thành một dòng hồi tố chứ không còn là sửa trực tiếp.")],
    ["net_paid",
     B("Net paid", "Đã chi"),
  ...
  payslips: [
    ...
    ["approved",
     B("Approved", "Đã duyệt"),
     B("How many payslips in this run have reached the done stage, against the total. A payslip carries its own position in the chain, which is not always the run's. That difference shows up here when one payslip has been held back for a question.",
  ...
  importwizard: [
    ["rows_loaded",
     ...
     B("How many rows the importer read out of the source. If this is not the row count of your file, the file was cut short or a sheet was missed. Finding that here is much better than finding it after the compute.",
       "Số dòng mà trình nhập liệu đọc được từ nguồn. Nếu con số này không bằng số dòng trong tệp của bạn thì tệp đã bị cắt hoặc bỏ sót một sheet. Phát hiện điều đó ở đây tốt hơn nhiều so với phát hiện sau khi đã tính lương.")],
    ["matched",
     B("Matched", "Đã khớp"),
    ...
    ["confidence_score",
     ...
     B("The share of rows read without ambiguity. It is not a grade and there is no pass mark. The number that matters is the count of rows underneath it: 98% of fifty rows still leaves one person unaccounted for.",
       "Tỷ lệ số dòng được đọc mà không nhập nhằng. Đây không phải điểm số và không có mức đạt. Con số quan trọng là số dòng bên dưới nó: 98% của năm mươi dòng vẫn còn một con người chưa được tính đến.")],
  ],

  fullfinal: [
    ["leavers_this_period",
     B("Leavers this period", "Thôi việc kỳ này"),
     B("Employees whose contract ends inside this period. Each one needs settling here AND excluding from the normal monthly run. Somebody in both is paid twice, and getting that back is a legal conversation rather than a payroll one.",
       "Những nhân viên có hợp đồng kết thúc trong kỳ này. Mỗi người vừa cần được quyết toán ở đây VÀ cần bị loại khỏi đợt lương tháng thông thường. Người nằm ở cả hai chỗ sẽ được trả hai lần, và đòi lại khoản đó là chuyện pháp lý chứ không còn là chuyện tính lương.")],
    ["pending_settlement",
  ...
  retro: [
    ...
    ["total_adjustment",
     ...
     B("The sum of the retro lines in this run. It is added on top of the month's own earnings. The run's net total will be higher than the same headcount normally produces, so say that out loud before somebody asks about the variance.",
       "Tổng của các dòng hồi tố trong đợt này. Nó được cộng thêm trên phần thu nhập của chính tháng đó. Tổng thực nhận của đợt sẽ cao hơn mức mà cùng sĩ số thường tạo ra, nên hãy nói điều đó ra trước khi có người hỏi về biến động.")],
    ["oldest_source_period",
     B("Oldest source period", "Kỳ gốc xa nhất"),
     B("The furthest-back month any line here is correcting. The older it is, the more reporting has already been done on the original figure. That is exactly why the correction is a retro line and never an edit to that month.",
       "Tháng xa nhất mà một dòng ở đây đang hiệu chỉnh. Càng lâu thì càng nhiều báo cáo đã được lập trên con số gốc. Đó chính là lý do việc sửa phải là một dòng hồi tố chứ không bao giờ là sửa vào tháng đó.")],
  ],
  ...
  dashboard: [
    ...
    ["pending_approval",
     ...
     B("How many PAYSLIPS are waiting at the HR or Finance tier, company-wide. Not runs. And not the Officer gate, which this tile does not count at all. It is a state, not your queue. How many are YOURS is a question the Approvals screen answers, in runs. Confusing the two means chasing work that was never yours.",
       "Có bao nhiêu PHIẾU LƯƠNG đang chờ ở cấp HR hoặc Tài chính, trên toàn công ty. Không phải số đợt lương. Và cũng không tính cổng Chuyên viên, ô này hoàn toàn không đếm cổng đó. Đây là một hiện trạng, không phải hàng đợi của bạn. Bao nhiêu là CỦA BẠN thì màn hình Phê duyệt mới trả lời, và trả lời theo số đợt. Nhầm lẫn hai thứ khiến bạn đi giục những phần việc vốn không phải của mình.")],
  ...
  approvals: [
    ["at_officer_review",
     B("At Officer review", "Ở cấp Chuyên viên"),
    ...
    ["at_hr_review",
     ...
     B("Runs the Officer has already passed, waiting at {{hrTierName}}. The lane names the tier that has to act next, which makes it the answer to \"who do I chase\" rather than a status word.",
       "Các đợt đã qua cấp Chuyên viên, đang chờ ở {{hrTierName}}. Làn này chỉ đích danh cấp phải xử lý tiếp theo, nên nó là câu trả lời cho \"tôi phải hỏi ai\" chứ không chỉ là một từ mô tả trạng thái.")],
    ["at_finance_approval",
     B("At Finance approval", "Ở cấp Tài chính"),
     B("The last gate before done. A run here has been read twice already. That is exactly why the reading at this tier is about totals rather than lines, and why a wrong line has to have been caught earlier.",
       "Cổng cuối cùng trước khi Hoàn tất. Một đợt ở đây đã được soát hai lần rồi. Chính vì vậy phần soát ở cấp này là soát các con số tổng chứ không soát từng dòng, và một dòng sai buộc phải bị bắt từ sớm hơn.")],
    ["net_at_stake",
     B("Net at stake", "Số tiền đang treo"),
     B("The net of every run in the pipeline, added together. This is money one or more signatures away from leaving the company's account, and anybody who finds a reason can still stop it. After the last gate, that stops being true.",
       "Tổng thực nhận của mọi đợt đang trong quy trình, cộng lại. Đây là số tiền chỉ còn cách tài khoản công ty một hoặc vài chữ ký, và bất kỳ ai tìm ra lý do đều còn kịp chặn lại. Sau cổng cuối cùng thì điều đó không còn đúng nữa.")],
  ],
    ...
    steps: [
      {
        ...
        say: {
          ...
          body: B("This is the Payobook dashboard. One screen. It says who you are, which pay run is live, and how much of it is still waiting on somebody.",
                  "Đây là trang tổng quan Payobook. Chỉ một màn hình. Nó cho biết bạn là ai, đợt lương nào đang chạy, và còn bao nhiêu phần đang chờ người khác."),
        },
      },
      {
      ...
      },
      {
        ...
        say: {
          ...
          body: B("Payobook does not compute pay from a fixed salary structure. Every payslip line comes from a named component. That component lives in a formula configuration you can open and read. Let us go and read one.",
                  "Payobook không tính lương bằng cấu trúc lương cố định. Mỗi dòng phiếu lương đến từ một thành phần có tên. Thành phần đó nằm trong một cấu hình công thức mà bạn mở ra đọc được. Chúng ta cùng đi đọc thử một cái."),
        },
      },
      {
      ...
      },
      {
        ...
        say: {
          ...
          body: B("Inputs, earnings, deductions and totals. A payslip line exists because a component here produced it. The coloured tag says which kind it is.",
                  "Đầu vào, thu nhập, khấu trừ và các tổng. Một dòng phiếu lương tồn tại vì có một thành phần ở đây sinh ra nó. Thẻ màu cho biết nó thuộc loại nào."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("No cell references. Components read by their own name, coloured by kind. Every chip is a link to the component behind it.",
                  "Không có địa chỉ ô. Các thành phần được gọi bằng chính tên của chúng, tô màu theo loại. Mỗi thẻ là một đường dẫn tới thành phần đứng sau nó."),
        },
      },
      {
      ...
      },
      {
        ...
        say: {
          ...
          body: B("Depends on and Used by tell you how far a rename or a deletion would reach. They are written down before you make one.",
                  "\"Phụ thuộc vào\" và \"Được dùng bởi\" cho biết một lần đổi tên hay xoá bỏ sẽ lan tới đâu. Chúng được viết ra trước khi bạn làm."),
        },
      },
      {
      ...
      },
      {
        ...
        say: {
          ...
          body: B("You can change a formula by describing the change in plain language. Whether that is switched on is a permission your administrator sets. It is not a property of the screen.",
                  "Bạn có thể đổi một công thức bằng cách mô tả thay đổi bằng lời. Việc này có được bật hay không là quyền do quản trị viên của bạn cấp. Đó không phải đặc tính của màn hình."),
      ...
      },
      {
        ...
        say: {
          ...
          body: B("This control builds a draft payslip for every eligible employee: gross, allowances, BHXH and thuế TNCN, net. Drafts only. Pressing it pays nothing, sends nothing and approves nothing.",
                  "Nút này dựng một phiếu lương nháp cho từng nhân viên đủ điều kiện: tổng thu nhập, phụ cấp, BHXH và thuế TNCN, thực nhận. Chỉ là bản nháp. Bấm nút này không chi tiền, không gửi và không phê duyệt gì cả."),
          tip: B("I am not pressing it. This walkthrough only reads; the pay-run scenario is where you press it yourself.",
                 "Tôi sẽ không bấm nút này. Lượt xem này chỉ để đọc; muốn tự bấm thì hãy mở kịch bản chạy đợt lương."),
        },
      },
      {
    ...
    steps: [
      {
        ...
        say: {
          ...
          body: B("The period decides which month is computed. The batch name is what everybody after you will search for. Read it before you move on.",
                  "Kỳ lương quyết định tháng nào được tính. Tên đợt là thứ những người sau bạn sẽ dùng để tìm. Hãy đọc nó trước khi đi tiếp."),
        },
      },
      {
      ...
      },
      {
        ...
        say: {
          ...
          body: B("Company, configuration and eligible headcount. If the headcount is not the number you expected, the answer is further back: a contract, or a person the import missed. It is much cheaper to find that here.",
                  "Công ty, cấu hình và số nhân viên đủ điều kiện. Nếu sĩ số không đúng như bạn nghĩ thì nguyên nhân nằm ở phía trước đó: một hợp đồng, hoặc một người mà đợt nhập liệu bỏ sót. Tìm ra điều đó ở đây rẻ hơn nhiều."),
          tip: B("Nobody is paid because they are on this list. They are paid because a payslip was computed, approved and sent. This is only the first of those three.",
      ...
      },
      {
        ...
        say: {
          ...
          body: B("This creates one draft payslip per eligible employee, computed by the configuration named above. Drafts only: nothing is paid, sent or approved. They are still real records, with real names on them.",
                  "Thao tác này tạo một phiếu lương nháp cho mỗi nhân viên đủ điều kiện, tính theo cấu hình đã nêu ở trên. Chỉ là bản nháp: chưa chi, chưa gửi, chưa duyệt. Nhưng chúng vẫn là bản ghi thật, mang tên người thật."),
          tip: B("Computed twice? The wizard replaces the drafts rather than doubling them. What it cannot undo is a run somebody has already submitted.",
                 "Lỡ tính hai lần? Trình hướng dẫn sẽ thay thế các bản nháp chứ không nhân đôi chúng. Cái nó không sửa được là một đợt lương đã có người trình phê duyệt."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("The third number is a question, not an error. The engine flags a payslip when something about it is unusual for this employee. It wants a person to look, not to fix.",
                  "Con số thứ ba là một câu hỏi, không phải một lỗi. Hệ thống gắn cờ một phiếu lương khi có gì đó bất thường so với chính nhân viên đó. Nó muốn có người nhìn qua, chứ không phải sửa."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("The run's name, its net total and its counts. From here the batch goes to the board. Somebody other than you moves it through its gates.",
                  "Tên đợt lương, tổng thực chi và các con số đếm. Từ đây lô này đi ra bảng đợt lương. Một người khác, không phải bạn, sẽ đẩy nó qua các cổng phê duyệt."),
        },
      },
    ],
  },
    ...
    steps: [
      ...
      },
      {
        ...
        say: {
          ...
          body: B("Division, period, headcount and net. The record's own state and your role decide which buttons a card offers, never the card itself. That is why two people see different footers on the same run.",
                  "Bộ phận, kỳ lương, sĩ số và số thực chi. Trạng thái của chính bản ghi và vai trò của bạn quyết định thẻ hiện nút nào, chứ không phải cái thẻ. Vì vậy hai người nhìn cùng một đợt lương có thể thấy hai bộ nút khác nhau."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("The selector at the top scopes the whole screen. Read a figure here without checking it, and you have read a figure about a batch you were not looking at.",
                  "Ô chọn ở trên cùng giới hạn phạm vi của cả màn hình. Đọc một con số ở đây mà không xem ô này thì bạn đang đọc con số của một lô khác."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("A dot marks a payslip the engine wants read. It is not a broken payslip. It is one whose numbers moved more than this employee's usually do.",
                  "Một chấm đánh dấu phiếu lương mà hệ thống muốn có người đọc. Đó không phải phiếu hỏng. Đó là phiếu có con số biến động nhiều hơn mức bình thường của chính nhân viên đó."),
        },
      },
      {
    ...
    steps: [
      {
        ...
        say: {
          ...
          body: B("Import a workbook of salary rules and Payobook converts every formula. Then it shows you exactly how clean the result is, before a single component is saved. To follow along, open a configuration's importer from Formula Studio and land on the resolution preview.",
                  "Nhập một bảng tính chứa các quy tắc lương và Payobook chuyển đổi từng công thức. Rồi nó cho bạn thấy chính xác kết quả sạch tới mức nào, trước khi lưu bất kỳ thành phần nào. Để đi theo, hãy mở trình nhập của một cấu hình từ Xưởng công thức và dừng ở trang xem trước kết quả."),
        },
      },
      {
        ...
        say: {
          ...
          body: B("The percentage comes from three things: how cleanly the formulas resolved, how many references survived intact, and how sensible the sample numbers look. A row in red points at something that could not be mapped, and quietly became zero. That is the exact trap this step catches.",
                  "Tỷ lệ phần trăm được tính từ ba thứ: mức độ sạch khi chuyển đổi công thức, số tham chiếu còn nguyên vẹn, và độ hợp lý của các con số mẫu. Dòng màu đỏ trỏ tới thứ không ánh xạ được, và đã lặng lẽ thành số không. Đó đúng là cái bẫy mà bước này bắt được."),
        },
      },
      {
