...
- **Admin**: `ash@biztinct.com` / `{withheld: rize-admin}` — reset in P0 under owner
  pre-authorization because the supplied password did not work. **Owner: change it.**
- Temporary validator (P0): `igc1.validator` / `{withheld: rize-p0-validator}` (uid 2065) — **deactivate**.
- Demo ESS logins given passwords for portal tests: `ess1.demo@payobook.com` / `{withheld: rize-ess-p7}`,
  `ess2.demo@payobook.com` / `{withheld: rize-ess-p2}` — originally passwordless by design; clear if wanted.
- Test actors (all @example.com, company 5 "Payobook Vietnam JSC"): `rize.p3.joiner` /
  `{withheld: rize-p3}`; `rize.p4.boss|leaver|zoho` / `{withheld: rize-p4}`; `rize.p6.hr|plain|alpha|lifecycle`
  / `{withheld: rize-p6}`; `rize.p8.mate` / `{withheld: rize-p8}`; `rize.p9.head|finance|plain|wfp|both` /
  `{withheld: rize-p9}`. Keep as demo actors or remove.
