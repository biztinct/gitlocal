...
- Tenant `abm` (AB Mauri, company 1: 153 employees, 24 departments, 65 jobs, 152 paid
  contracts, payslips to 2026-06-30). Login `ash@biztinct.com` / `{withheld: abm-admin}`.
...
- WF15 (P1): **the abm admin password in this ledger is WRONG.** `ash@biztinct.com` /
  `{withheld: abm-admin}` is refused, and so is every obvious variant. Browser validation on
