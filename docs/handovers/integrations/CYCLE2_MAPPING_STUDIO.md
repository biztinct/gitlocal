...
- Ledger is now binding through **W130**. Three that bite this cycle: **W127** — the JS pre-rsync gate is `node --input-type=module --check < file` (the plain file form is a no-op on OWL files); **W129** — `ash@biztinct.com/{withheld: apex-admin-2026-08}` is STALE since 2026-08-12; for live validation create a temporary SINGLE-company system user via `odoo-bin shell` (`company_ids` and `company_id` in the SAME write), use it, remove it in the same session — never touch existing users' passwords; **W130** — a second session drives its OWN Chrome over CDP with `--remote-allow-origins`, and a browser doesn't have to log in through the form (session cookie via shell-minted key works when the form is unavailable).
