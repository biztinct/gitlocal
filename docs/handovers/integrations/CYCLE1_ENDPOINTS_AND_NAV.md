...
- Chrome MCP: standing approval to run/restart it. Live backend `https://payobook.com` — admin `ash@biztinct.com` / `{withheld: apex-admin-2026-08}` (company 5 "Payobook Vietnam JSC" is the demo world; companies 2/6/7 archived). Demo user `demo@payobook.com` / `{withheld: demo-user}`.
