# -*- coding: utf-8 -*-
"""B1 on a database that already has this module.

`post_init_hook` fires on INSTALL ONLY and never on `-u` — the Odoo 19 trap the
ledger records (AM70) — so anything an install does has to be done here as
...
anything. Neither is ever fatal — an upgrade that refuses because a default
route could not be laid is a worse outcome than a route somebody lays by hand a
minute later.
"""
...
def _route(env):
    try:
        from odoo.addons.pb_goals.models.goal_set_approval import seed_all
        laid = seed_all(env)
        _logger.info('pb_goals: the goal-sheet route was laid for %s '
                     'compan(ies)', laid)
    except Exception:                   # noqa: BLE001 — never fail an upgrade
        _logger.warning('pb_goals: the goal-sheet route could not be seeded',
                        exc_info=True)
