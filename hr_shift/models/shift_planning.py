...
class ShiftPlanningShift(models.Model):
    ...
    def action_view_shift_details(self):
        ...
        shift_line_id = self.env.context.get("shift_line_id")
        if shift_line_id:
            action["view_mode"] = "form"
            action["views"] = [(False, "form")]
            # Convert to int - QWeb context passes string values
            action["res_id"] = int(shift_line_id) if isinstance(shift_line_id, str) else shift_line_id
            action["target"] = "new"
