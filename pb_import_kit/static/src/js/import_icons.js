/** @odoo-module **/
// Shared Lucide icon set + ic() helper for every Payobook Import surface.
// Lucide-style paths (24x24, currentColor stroke). NEVER use emoji.
...
export const IC = {
    ...
    // Lucide `git-merge` — two sources joining one target: the Mapping Studio's
    // whole sentence in one glyph (Integrations Cycle 2).
    ...
    // FLEET P6. The way OUT of a support session. Its twin has been here since
    // the kit was written; the pair belongs together rather than in one
    // module's private set, because "leave" is not a platform-only idea.
    logOut:     '<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/>',
    filter:     '<polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/>',
    clock:      '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>',
    loader:     '<path d="M21 12a9 9 0 1 1-6.219-8.56"/>',
    shieldCheck:'<path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="m9 12 2 2 4-4"/>',
    // workforce / Time hub (P1a) — one shared registry, no per-module icon files (W2)
    activity:   '<path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2"/>',
    plane:      '<path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z"/>',
    // Today board (P1b) — the map card + its empty state
    ...
    // IA redesign C1 — the global ⌘K palette names every surface in the product,
    // so nine glyphs that only existed in `pb_sidebar.js`'s own closed rail set
    // (or nowhere) join the shared registry. W2: one place, never a per-module
    // map. Paths are Lucide's, matched to the names the rail already uses so the
    // palette row and the rail item show the SAME picture of a surface.
    ...
    // IA Cycle 5: the last nine Font Awesome glyphs in the product were in
    // pb_bank_ocr, and eight of them had an equivalent already in this registry.
    // `pencil` is the ninth — a bank account changed BY HAND, beside the `scan`
    // that says it was read off a document. New icons are ADDED here rather than
    // to a per-module file (W2).
    ...
    // sentiment (P8 — the Today board's Team pulse tile). Lucide faces, so a
    // BACKEND surface never has to reach for an emoji: the portal's five-face
    // rating control is the data itself and may use glyphs, a manager's tile is
    // chrome and may not.
    ...
    // (W2), and Lucide paths only — no emoji, no FontAwesome.
    ...
    // GROUP P5 — people in two places. `circle` is the UNCHOSEN half of a
    // pair whose chosen half is `checkCircle`: a set of options where only
    // the picked one carries a mark reads as a list with one tick in it,
    // and the two together read as a choice. `userCheck` is two employee
