# -*- coding: utf-8 -*-
"""Approval Matrix P6 — pb_rnr's default approval route, on an upgrade.
...
def migrate(cr, version):
    ...
    from odoo.addons.pb_rnr.models.nomination_approval import seed_all
    try:
        seed_all(env)
    except Exception:       # noqa: BLE001 — an upgrade must not die on a seed
        _logger.exception('pb_rnr: the default approval route could not '
                          'be seeded')
